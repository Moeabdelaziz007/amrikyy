...
class TaskExecutor {
  ...
  async executeTask(task, agentName) {
    ...
    try {
      console.log(
        `⚡ Executing task ${task.id} with ${agentName} (Execution: ${executionId})`
      );
  ...
  cleanupStuckTasks() {
    ...
    this.executionPool.forEach((execution, id) => {
      if (
        ...
        now - execution.startTime > 300000
      ) {
        stuckTasks.push(id);
      }
    });
  ...
  updateExecutionStats() {
    const successRate =
      this.executionStats.totalExecuted > 0
        ? (
            (this.executionStats.successfulExecutions /
              this.executionStats.totalExecuted) *
            100
    ...
    console.log(
      `📊 Execution Stats: ${this.executionStats.successfulExecutions}/${this.executionStats.totalExecuted} (${successRate}%)`
    );
    console.log(
      `⏱️ Average execution time: ${Math.round(this.executionStats.averageExecutionTime)}ms`
    );
  }
