...
exports.AIPromptManager = void 0;
exports.getAIPromptManager = getAIPromptManager;
class AIPromptManager {
    ...
    initializeDefaultPrompts() {
        ...
        this.registerPrompt({
            ...
            prompt: `Act as a professional content creator and social media expert. Create engaging, high-quality content for social media platforms that will increase engagement and reach. The content should be professional and friendly, target your target audience, and include relevant hashtags and call-to-actions. Consider current trends and best practices for social media content creation.`,
            variables: [
                {
                    name: 'platform',
        ...
        this.registerPrompt({
            ...
            prompt: `Act as a nutritionist and create a healthy recipe for a vegan dinner. Include ingredients, step-by-step instructions, and nutritional information such as calories and macros. Consider any dietary restrictions and ensure the recipe is easy to prepare.`,
            variables: [
                {
                    name: 'mealType',
    ...
    searchPrompts(query) {
        ...
        return Array.from(this.prompts.values()).filter(prompt => prompt.title.toLowerCase().includes(lowercaseQuery) ||
            prompt.description.toLowerCase().includes(lowercaseQuery) ||
            prompt.tags.some(tag => tag.toLowerCase().includes(lowercaseQuery)) ||
            prompt.category.toLowerCase().includes(lowercaseQuery));
    }
    // Prompt Execution
    async executePrompt(promptId, variables, userId) {
    ...
    async executeTemplate(templateId, variables, userId) {
    ...
    mapOutputs(results, outputMapping) {
        const mapped = {};
    ...
    // Usage Analytics
    updatePromptUsage(promptId, userId) {
        const prompt = this.getPrompt(promptId);
        if (!prompt)
            return;
    ...
    // Feedback System
    submitFeedback(promptId, feedback) {
        const prompt = this.getPrompt(promptId);
        if (!prompt)
            return false;
...
exports.AIPromptManager = AIPromptManager;
// Export singleton instance
let aiPromptManager = null;
function getAIPromptManager() {
    if (!aiPromptManager) {
