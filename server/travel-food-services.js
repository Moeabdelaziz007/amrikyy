...
exports.TravelFoodServiceManager = void 0;
exports.getTravelFoodServiceManager = getTravelFoodServiceManager;
const schema_js_1 = require("../shared/schema.js");
const storage_js_1 = require("./storage.js");
const drizzle_orm_1 = require("drizzle-orm");
// Helper function to calculate AI score based on persona
const calculateAIScore = (service, persona) => {
    if (!persona) {
...
class TravelFoodServiceManager {
    travelServices = new Map();
    foodServices = new Map();
    smartAgents = new Map();
    constructor() {
    ...
    createCustomAgentTemplate(name, category, capabilities, integrations, automationRules) {
    ...
    async generateTravelRecommendations(userId, destination, budget, preferences) {
        const user = await storage_js_1.storage.select().from(schema_js_1.users).where((0, drizzle_orm_1.eq)(schema_js_1.users.id, userId)).first();
    ...
    async generateFoodRecommendations(userId, location, budget, preferences) {
        const user = await storage_js_1.storage.select().from(schema_js_1.users).where((0, drizzle_orm_1.eq)(schema_js_1.users.id, userId)).first();
        const persona = user?.persona ?? null;
...
exports.TravelFoodServiceManager = TravelFoodServiceManager;
// Export singleton instance
let travelFoodServiceManager = null;
function getTravelFoodServiceManager() {
