...
class FreeAITools {
     ...
     */
    static async analyzeText(text, analysisType) {
        try {
            let model = '';
            switch (analysisType) {
                case 'sentiment':
            ...
            const response = await axios_1.default.post(`${this.FREE_APIS.HUGGINGFACE_API}/${model}`, { inputs: text }, {
                headers: {
     ...
     */
    static async fallbackTextAnalysis(text, analysisType) {
        switch (analysisType) {
            case 'sentiment':
    ...
    static simpleKeywordExtraction(text) {
    ...
    static simpleSummarization(text) {
    ...
    static async translateText(text, targetLang, sourceLang = 'auto') {
        try {
            const response = await axios_1.default.post(this.FREE_APIS.LIBRE_TRANSLATE, {
                q: text,
                source: sourceLang,
                target: targetLang,
                format: 'text',
            ...
            try {
                const fallbackResponse = await axios_1.default.get(this.FREE_APIS.MYMEMORY_TRANSLATE, {
                    params: {
                        q: text,
    ...
    static simpleTranslation(text, targetLang) {
     ...
     */
    static async processImage(imageUrl, operation) {
        try {
            switch (operation) {
                case 'analyze':
    ...
    static async resizeImage(imageUrl) {
    ...
    static async applyFilter(imageUrl) {
     ...
     * Free Data Analysis
     */
    static async analyzeData(data, analysisType) {
        try {
            switch (analysisType) {
                case 'statistics':
    ...
    static calculateRSquared(x, y, slope, intercept) {
     ...
     */
    static async generateCode(language, description, template) {
        try {
            const codeTemplates = {
                javascript: {
     ...
     */
    static async saveAnalysisToFirestore(userId, analysisType, results) {
        try {
            const analysisData = {
                type: analysisType,
                results,
            ...
            return await firebase_1.FirestoreService.createPost(userId, analysisData);
        }
        catch (error) {
...
exports.FreeAITools = FreeAITools;
exports.default = FreeAITools;
