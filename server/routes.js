...
const smart_telegram_bot_js_1 = require("./smart-telegram-bot.js");
const travel_food_services_js_1 = require("./travel-food-services.js");
...
const real_time_streaming_js_1 = require("./real-time-streaming.js");
const ai_model_management_js_1 = require("./ai-model-management.js");
const learning_automation_js_1 = require("./learning-automation.js");
const enterprise_team_management_js_1 = require("./enterprise-team-management.js");
const enterprise_admin_dashboard_js_1 = require("./enterprise-admin-dashboard.js");
const enterprise_collaboration_js_1 = require("./enterprise-collaboration.js");
const enhanced_travel_agency_js_1 = require("./enhanced-travel-agency.js");
const travel_dashboard_js_1 = require("./travel-dashboard.js");
const n8n_node_system_js_1 = require("./n8n-node-system.js");
const n8n_integrations_js_1 = require("./n8n-integrations.js");
const ai_prompt_manager_js_1 = require("./ai-prompt-manager.js");
async function registerRoutes(app) {
    ...
    if (telegramToken) {
        try {
            ...
            (0, telegram_js_1.initializeTelegramBot)(telegramToken);
            const smartBot = (0, smart_telegram_bot_js_1.initializeSmartTelegramBot)(telegramToken);
    ...
    try {
        await (0, mcp_protocol_js_1.initializeMCP)();
    ...
    try {
        const aiToolsManager = (0, advanced_ai_tools_js_1.getAdvancedAIToolsManager)();
    ...
    try {
        const aiAgentSystem = (0, advanced_ai_agents_js_1.getAdvancedAIAgentSystem)();
    ...
    try {
        const multiModalAI = (0, multi_modal_ai_js_1.initializeMultiModalAI)();
    ...
    try {
        const realTimeStreaming = (0, real_time_streaming_js_1.initializeRealTimeAIStreaming)();
    ...
    try {
        const modelManagement = (0, ai_model_management_js_1.initializeAIModelManagement)();
    ...
    try {
        const learningSystem = (0, learning_automation_js_1.initializeLearningSystem)();
    ...
    const learningSystem = (0, learning_automation_js_1.getLearningSystem)();
    ...
    const adminDashboard = (0, enterprise_admin_dashboard_js_1.getEnterpriseAdminDashboard)();
    const collaborationSystem = (0, enterprise_collaboration_js_1.getEnterpriseCollaborationSystem)();
    const travelAgency = (0, enhanced_travel_agency_js_1.getEnhancedTravelAgency)();
    const travelDashboard = (0, travel_dashboard_js_1.getTravelDashboard)();
    ...
    app.get('/api/users/current', async (req, res) => {
        try {
            ...
            const user = await storage_1.storage.getUser('user-1');
            if (!user) {
    ...
    app.get('/api/users/:id/stats', async (req, res) => {
        try {
            const { id } = req.params;
            const stats = await storage_1.storage.getUserStats(id);
            res.json(stats);
        }
        catch (error) {
    ...
    app.get('/api/posts', async (req, res) => {
        try {
            const limit = parseInt(req.query.limit) || 10;
            const posts = await storage_1.storage.getPostsWithAuthor(limit);
            res.json(posts);
        }
        catch (error) {
    ...
    app.post('/api/posts', async (req, res) => {
        try {
            const validatedData = schema_js_1.insertPostSchema.parse(req.body);
            const post = await storage_1.storage.createPost(validatedData);
    ...
    app.post('/api/posts/:id/like', async (req, res) => {
        try {
            const { id } = req.params;
            const post = await storage_1.storage.getPost(id);
            if (!post) {
                return res.status(404).json({ message: 'Post not found' });
            }
            const newLikes = post.likes + 1;
            await storage_1.storage.updatePostStats(id, newLikes, post.shares, post.comments);
    ...
    app.get('/api/agent-templates', async (req, res) => {
        try {
            const templates = await storage_1.storage.getAgentTemplates();
            res.json(templates);
        }
        catch (error) {
    ...
    app.post('/api/agent-templates/:id/use', async (req, res) => {
        try {
            const { id } = req.params;
            await storage_1.storage.incrementTemplateUsage(id);
    ...
    app.get('/api/user-agents', async (req, res) => {
        try {
            ...
            const agents = await storage_1.storage.getUserAgents(userId);
            res.json(agents);
        }
        catch (error) {
    ...
    app.post('/api/user-agents', async (req, res) => {
        try {
            const validatedData = schema_js_1.insertUserAgentSchema.parse(req.body);
            const agent = await storage_1.storage.createUserAgent(validatedData);
    ...
    app.get('/api/workflows', async (req, res) => {
        try {
            const userId = req.query.userId || 'user-1';
            const workflows = await storage_1.storage.getWorkflowsByUser(userId);
            res.json(workflows);
        }
        catch (error) {
    ...
    app.post('/api/workflows', async (req, res) => {
        try {
            const validatedData = schema_js_1.insertWorkflowSchema.parse(req.body);
            const workflow = await storage_1.storage.createWorkflow(validatedData);
    ...
    app.put('/api/workflows/:id', async (req, res) => {
        try {
            const { id } = req.params;
            await storage_1.storage.updateWorkflow(id, req.body);
            const workflow = await storage_1.storage.getWorkflow(id);
            res.json(workflow);
        }
        catch (error) {
    ...
    app.delete('/api/workflows/:id', async (req, res) => {
        try {
            const { id } = req.params;
            await storage_1.storage.deleteWorkflow(id);
    ...
    app.post('/api/chat', async (req, res) => {
        try {
            ...
            // Save chat message
            await storage_1.storage.createChatMessage({
                userId,
                message,
    ...
    app.get('/api/chat/history', async (req, res) => {
        try {
            ...
            const messages = await storage_1.storage.getChatMessages(userId, limit);
            res.json(messages);
        }
        catch (error) {
    ...
    app.get('/api/telegram/status', async (req, res) => {
        try {
            const telegramService = (0, telegram_js_1.getTelegramService)();
            if (!telegramService) {
    ...
    app.post('/api/ai/smart-learning', async (req, res) => {
        try {
            ...
            const smartAI = (0, smart_learning_ai_js_1.getSmartLearningAI)();
            const context = {
                userId,
    ...
    app.get('/api/travel/services', async (req, res) => {
        try {
            const travelServiceManager = (0, travel_food_services_js_1.getTravelFoodServiceManager)();
    ...
    app.post('/api/travel/recommendations', async (req, res) => {
        try {
            ...
            const travelServiceManager = (0, travel_food_services_js_1.getTravelFoodServiceManager)();
            const recommendations = await travelServiceManager.generateTravelRecommendations(destination, budget, preferences || {});
            res.json(recommendations);
        }
        catch (error) {
    ...
    app.get('/api/food/services', async (req, res) => {
        try {
            const travelFoodServiceManager = (0, travel_food_services_js_1.getTravelFoodServiceManager)();
    ...
    app.post('/api/food/recommendations', async (req, res) => {
        try {
            ...
            const travelFoodServiceManager = (0, travel_food_services_js_1.getTravelFoodServiceManager)();
            const recommendations = await travelFoodServiceManager.generateFoodRecommendations(location, budget, preferences || {});
            res.json(recommendations);
        }
        catch (error) {
    ...
    app.post('/api/shopping/agents', async (req, res) => {
        try {
            ...
            const travelFoodServiceManager = (0, travel_food_services_js_1.getTravelFoodServiceManager)();
            const agent = travelFoodServiceManager.createCustomAgentTemplate(name, category, capabilities || {}, integrations || [], automationRules || []);
            res.status(201).json(agent);
        }
        catch (error) {
    ...
    app.post('/api/automation/travel-workflow', async (req, res) => {
        try {
            ...
            await storage_1.storage.createWorkflow({
                userId: 'user-1',
                name: workflow.name,
    ...
    app.get('/api/telegram/smart/status', async (req, res) => {
        try {
            const smartBot = (0, smart_telegram_bot_js_1.getSmartTelegramBot)();
            if (!smartBot) {
    ...
    app.post('/api/mcp/message', async (req, res) => {
        try {
            ...
            const mcpProtocol = (0, mcp_protocol_js_1.getMCPProtocol)();
    ...
    app.post('/api/ai-agents/collaboration', async (req, res) => {
        try {
            ...
            const aiAgentSystem = (0, advanced_ai_agents_js_1.getAdvancedAIAgentSystem)();
            const collaboration = await aiAgentSystem.createCollaboration(agents, task, coordination || 'parallel');
    ...
    app.get('/api/ai/multimodal/models', async (req, res) => {
        try {
            const multiModalAI = (0, multi_modal_ai_js_1.getMultiModalAIEngine)();
    ...
    app.get('/api/ai/streaming/status', async (req, res) => {
        try {
            const streaming = (0, real_time_streaming_js_1.getRealTimeAIStreaming)();
    ...
    app.get('/api/ai/models', async (req, res) => {
        try {
            const modelManagement = (0, ai_model_management_js_1.getAIModelManagementSystem)();
    ...
    app.post('/api/collaboration/sessions', async (req, res) => {
        try {
            ...
            const session = await collaborationSystem.createCollaborationSession(type, resourceId, resourceName, teamId, creatorId, permissions);
    ...
    app.get('/api/system/status', async (req, res) => {
        try {
            ...
            const agentSystem = (0, advanced_ai_agents_js_1.getAdvancedAIAgentSystem)();
            const aiInfo = {
    ...
    app.post('/api/ai/chat', async (req, res) => {
        try {
            ...
            const response = await (0, gemini_js_1.chatWithAssistant)(message, context);
            res.json({
                response: response,
                timestamp: new Date().toISOString(),
                context: context
    ...
    app.get('/api/workflows/templates', async (req, res) => {
        try {
            const workflowOrchestrator = (0, intelligent_workflow_js_1.getIntelligentWorkflowOrchestrator)();
    ...
    const n8nNodeSystem = (0, n8n_node_system_js_1.getN8nNodeSystem)();
    console.log('🔧 N8n Node System initialized');
    // Initialize N8n Integration Manager
    const n8nIntegrationManager = (0, n8n_integrations_js_1.getN8nIntegrationManager)();
    console.log('🔌 N8n Integration Manager initialized');
    // Initialize AI Prompt Manager
    const aiPromptManager = (0, ai_prompt_manager_js_1.getAIPromptManager)();
    ...
    app.get('/api/integrations/connectors/category/:category', (req, res) => {
        try {
            const connectors = n8nIntegrationManager.getConnectorsByCategory(req.params.category);
            res.json({ success: true, connectors });
        }
        catch (error) {
            res.status(500).json({ success: false, error: error.message });
    ...
    app.get('/api/ai/prompts/category/:category', (req, res) => {
        try {
            const prompts = aiPromptManager.getPromptsByCategory(req.params.category);
            res.json({ success: true, prompts });
        }
        catch (error) {
            res.status(500).json({ success: false, error: error.message });
