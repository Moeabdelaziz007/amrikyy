...
import { pgTable, text, timestamp, uuid, jsonb, boolean, integer, real, pgEnum } from 'drizzle-orm/pg-core';
import { relations } from 'drizzle-orm';

// Enums
export const userRoleEnum = pgEnum('user_role', ['admin', 'user', 'viewer']);
export const taskTypeEnum = pgEnum('task_type', ['workflow', 'trigger', 'action', 'condition', 'ai', 'mcp']);
export const taskCategoryEnum = pgEnum('task_category', ['social_media', 'email', 'data', 'ai', 'integration', 'notification']);
export const taskStatusEnum = pgEnum('task_status', ['active', 'inactive', 'draft', 'error', 'running', 'paused']);
export const taskPriorityEnum = pgEnum('task_priority', ['low', 'medium', 'high', 'critical']);
export const executionStatusEnum = pgEnum('execution_status', ['pending', 'running', 'completed', 'failed', 'cancelled', 'retrying']);
export const logLevelEnum = pgEnum('log_level', ['info', 'warning', 'error', 'debug']);
export const mcpToolCategoryEnum = pgEnum('mcp_tool_category', ['development', 'data', 'database', 'web', 'ai', 'automation', 'integration']);
export const mcpToolStatusEnum = pgEnum('mcp_tool_status', ['active', 'inactive', 'error', 'updating']);
export const suggestionCategoryEnum = pgEnum('suggestion_category', ['optimization', 'automation', 'integration', 'performance', 'security', 'scalability']);
export const suggestionPriorityEnum = pgEnum('suggestion_priority', ['low', 'medium', 'high', 'critical']);
export const suggestionImpactEnum = pgEnum('suggestion_impact', ['low', 'medium', 'high', 'transformational']);
export const suggestionEffortEnum = pgEnum('suggestion_effort', ['low', 'medium', 'high', 'extensive']);
export const alertTypeEnum = pgEnum('alert_type', ['info', 'warning', 'error', 'critical']);
...
export const nodeTypeEnum = pgEnum('node_type', ['trigger', 'action', 'condition', 'delay', 'webhook', 'ai', 'data', 'loop', 'merge']);
export const nodeStatusEnum = pgEnum('node_status', ['idle', 'running', 'success', 'error', 'warning']);
export const portTypeEnum = pgEnum('port_type', ['string', 'number', 'boolean', 'object', 'array', 'any']);
export const connectionTypeEnum = pgEnum('connection_type', ['data', 'control', 'conditional']);
export const themeEnum = pgEnum('theme', ['light', 'dark', 'auto']);
export const layoutEnum = pgEnum('layout', ['grid', 'list', 'kanban']);
export const sortByEnum = pgEnum('sort_by', ['name', 'date', 'priority', 'status']);
export const groupByEnum = pgEnum('group_by', ['none', 'status', 'priority', 'category']);
export const errorHandlingEnum = pgEnum('error_handling', ['stop', 'continue', 'retry']);
...
export const taskExecutions = pgTable('task_executions', {
  id: uuid('id').primaryKey().defaultRandom(),
  taskId: uuid('task_id').notNull().references(() => automationTasks.id, { onDelete: 'cascade' }),
  userId: uuid('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),
...
export const executionLogs = pgTable('execution_logs', {
  id: uuid('id').primaryKey().defaultRandom(),
  executionId: uuid('execution_id').notNull().references(() => taskExecutions.id, { onDelete: 'cascade' }),
...
export const workflowSuggestions = pgTable('workflow_suggestions', {
  ...
  estimatedSavings: jsonb('estimated_savings').$type<{
    ...
    cost: number;
    resources: number;
  }>().notNull().default({ time: 0, cost: 0, resources: 0 }),
...
export const automationTasksRelations = relations(automationTasks, ({ one, many }) => ({
  workspace: one(workspaces, {
...
export const toolIntegrationsRelations = relations(toolIntegrations, ({ one }) => ({
  tool: one(mcpTools, {
