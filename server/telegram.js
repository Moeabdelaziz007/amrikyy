...
const smart_menu_js_1 = require("./smart-menu.js");
class TelegramService {
    bot;
    isConnected = false;
    smartMenuService = (0, smart_menu_js_1.getSmartMenuService)();
    constructor(token) {
    ...
    async handleMessage(msg) {
        ...
        await storage_js_1.storage.createChatMessage({
    ...
    // Handle smart menu callbacks
    async handleSmartMenuCallback(chatId, callbackData) {
    ...
    async sendSmartWelcomeMessage(chatId, username) {
    ...
    // Smart help message
    async sendSmartHelpMessage(chatId) {
...
🎯 Try /menu to see your personalized smart menu!`;
    ...
    // Send smart menu
    async sendSmartMenu(chatId, username) {
        const menu = await this.smartMenuService.generateSmartMenu(chatId, username, 'main');
        await this.bot.sendMessage(chatId, menu.text, menu.keyboard);
    }
...
💡 Pro tip: Send any message and I'll help you with AI-powered content suggestions.`;
    ...
    async sendStatusMessage(chatId) {
        try {
            const stats = await storage_js_1.storage.getUserStats('user-1');
            const posts = await storage_js_1.storage.getPostsWithAuthor(5);
            const agents = await storage_js_1.storage.getUserAgents('user-1');
...
📱 Recent Activity:
${posts.slice(0, 3).map(post => `• ${post.content.substring(0, 50)}... (${post.likes} likes)`).join('\n')}
...
🤖 Active Agents: ${agents.filter(a => a.isActive).length}`;
    ...
    async sendAgentTemplates(chatId) {
        try {
            const templates = await storage_js_1.storage.getAgentTemplates();
    ...
    async sendDefaultResponse(chatId, text) {
        if (!text)
            return;
    ...
    async createPostFromTelegram(chatId, content) {
        try {
            const post = await storage_js_1.storage.createPost({
                authorId: 'user-1',
                content: content,
                isAiGenerated: false
            });
    ...
    async schedulePostFromTelegram(chatId, time, content) {
