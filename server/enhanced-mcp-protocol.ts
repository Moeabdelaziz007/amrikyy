...
export class EnhancedMCPProtocol extends EventEmitter {
  ...
  private initializeFreeTools() {
    ...
    this.addTool({
      ...
      execute: async params => {
        return await this.scrapeWebContent(
          params.url,
          params.selector,
          params.format
        );
    ...
    this.addTool({
      ...
      execute: async params => {
        return await this.processCsvData(
          params.csvData,
          params.operation,
          params.filterColumn,
          params.filterValue
  ...
  private async scrapeWebContent(
    url: string,
    selector?: string,
    format: string = 'text'
  ): Promise<any> {
  ...
  public async executeTool(name: string, params: any): Promise<any> {
    ...
    if (tool.rateLimit) {
      ...
      const current = this.rateLimits.get(limitKey) || {
        count: 0,
        resetTime: now + 60000,
      };
