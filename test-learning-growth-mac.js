...
class MacLearningGrowthTester {
  ...
  async startMacLearningTest() {
    ...
    if (!window.chatbotSystem) {
      console.log(
        '⚠️ Chatbot system not found. Creating Mac-optimized mock system...'
      );
    ...
    console.log(
      '✅ Mac learning test started! Check console for real-time metrics.'
    );
  ...
  detectMacPerformance() {
    ...
    console.log(
      `  🧠 Apple Silicon: ${isAppleSilicon ? 'Yes (M1/M2)' : 'No (Intel)'}`
    );
  ...
  async runMacOptimizedLearningTest() {
    ...
    while (this.isRunning) {
      const message =
        this.testMessages[messageIndex % this.testMessages.length];
  ...
  async simulateMacMessageProcessing(message) {
    ...
    if (!chatbot.zeroShotCache.has(cacheKey)) {
      chatbot.zeroShotCache.set(
        cacheKey,
        `Mac-cached response for: ${message}`
      );
    }
    ...
    if (chatbot.metaLearningModel.contextMemory.length > 100) {
      chatbot.metaLearningModel.contextMemory =
        chatbot.metaLearningModel.contextMemory.slice(-50);
    }
  ...
  updateMacLearningMetrics(message, response, responseTime) {
    ...
    this.learningMetrics.adaptationRules =
      chatbot.metaLearningModel.adaptationRules.size;
    this.learningMetrics.contextMemoryItems =
      chatbot.metaLearningModel.contextMemory.length;
    ...
    this.learningMetrics.responseAccuracy = Math.min(
      0.98,
      this.learningMetrics.responseAccuracy + macAccuracyBoost
    );
  ...
  logMacProgress() {
    ...
    console.log(
      `🍎 Mac Progress: ${this.messageCount} messages processed in ${elapsed.toFixed(1)}s (${messagesPerSecond.toFixed(2)} msg/s)`
    );
    console.log(`🚀 Mac Performance Mode: ${this.macPerformanceMode}`);
  }
  ...
  logMacDetailedMetrics() {
    ...
    console.log(
      `  📨 Messages processed: ${this.messageCount} (${messagesPerSecond.toFixed(2)} msg/s)`
    );
    console.log(
      `  🎯 Patterns learned: ${this.learningMetrics.patternsLearned}`
    );
    console.log(
      `  💾 Zero-shot cache: ${this.learningMetrics.zeroShotCacheSize} items`
    );
    console.log(
      `  🔄 Adaptation rules: ${this.learningMetrics.adaptationRules}`
    );
    console.log(
      `  🧠 Context memory: ${this.learningMetrics.contextMemoryItems} items`
    );
    console.log(
      `  📈 Learning rate: ${(this.learningMetrics.learningRate * 100).toFixed(1)}%`
    );
    console.log(
      `  ✅ Response accuracy: ${(this.learningMetrics.responseAccuracy * 100).toFixed(1)}%`
    );
    console.log(
      `  ⚡ Avg response time: ${this.learningMetrics.averageResponseTime.toFixed(0)}ms`
    );
    console.log('🍎 Mac-Specific Optimizations:');
    console.log(
      `  Metal Performance: ${this.learningMetrics.macOptimizations.metalPerformance.toFixed(1)}`
    );
    console.log(
      `  🧠 CoreML Integration: ${this.learningMetrics.macOptimizations.coreMLIntegration.toFixed(1)}`
    );
    console.log(
      `  🔗 Swift Bridge Calls: ${this.learningMetrics.macOptimizations.swiftBridgeCalls}`
    );
    console.log(
      `  💾 Native Memory Usage: ${this.learningMetrics.macOptimizations.nativeMemoryUsage.toFixed(2)}`
    );
    console.log('---');
  }
  ...
  generateMacFinalReport() {
    ...
    console.log(
      `🚀 Processing rate: ${messagesPerSecond.toFixed(2)} messages/second`
    );
    console.log(
      `🎯 Final patterns learned: ${this.learningMetrics.patternsLearned}`
    );
    console.log(
      `💾 Final cache size: ${this.learningMetrics.zeroShotCacheSize}`
    );
    console.log(
      `🔄 Final adaptation rules: ${this.learningMetrics.adaptationRules}`
    );
    console.log(
      `🧠 Final context memory: ${this.learningMetrics.contextMemoryItems} items`
    );
    console.log(
      `📈 Final learning rate: ${(this.learningMetrics.learningRate * 100).toFixed(1)}%`
    );
    console.log(
      `✅ Final accuracy: ${(this.learningMetrics.responseAccuracy * 100).toFixed(1)}%`
    );
    console.log(
      `⚡ Final avg response time: ${this.learningMetrics.averageResponseTime.toFixed(0)}ms`
    );

    // Calculate Mac-optimized learning growth per second
    const patternsPerSecond = this.learningMetrics.patternsLearned / totalTime;
    const cacheGrowthPerSecond =
      this.learningMetrics.zeroShotCacheSize / totalTime;
    const rulesPerSecond = this.learningMetrics.adaptationRules / totalTime;

    console.log('📊 Mac-Optimized Learning Growth Rates:');
    console.log(
      `  🎯 Patterns learned per second: ${patternsPerSecond.toFixed(3)}`
    );
    console.log(
      `  💾 Cache growth per second: ${cacheGrowthPerSecond.toFixed(3)} items`
    );
  ...
  exportMacResults() {
    const report = {
      ...
      messagesPerSecond:
        this.messageCount / ((Date.now() - this.startTime) / 1000),
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  console.log(
    '  window.macLearningTester.startMacLearningTest() - Start Mac learning test'
  );
  console.log(
    '  window.macLearningTester.stopMacLearningTest() - Stop Mac learning test'
  );
  console.log(
    '  window.macLearningTester.logMacDetailedMetrics() - Show current Mac metrics'
  );
