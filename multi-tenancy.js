...
class MultiTenancySystem {
  ...
  async loadTenants() {
    try {
      const tenantsSnapshot = await firebase
        .firestore()
        .collection('tenants')
        .get();
  ...
  populateTenantList() {
    ...
    const tenants = Array.from(this.tenants.values()).filter(
  ...
  createTenantModals() {
    ...
    document
      .getElementById('submitCreateTenant')
  ...
  async getTenantUsage(tenantId) {
    try {
      ...
      const apiCallsSnapshot = await firebase
        .firestore()
        .collection('analytics')
        .where('tenantId', '==', tenantId)
