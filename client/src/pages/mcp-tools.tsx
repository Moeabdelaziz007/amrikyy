          ...
          <div className="p-6 max-w-7xl mx-auto">
            <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <tabs_1.TabsList className="grid w-full grid-cols-3">
                ...
                <tabs_1.TabsTrigger value="documentation">Documentation</tabs_1.TabsTrigger>
              </tabs_1.TabsList>

              <tabs_1.TabsContent value="overview" className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {categories.map(category => (<card_1.Card key={category} className="glass-card">
                      <card_1.CardHeader>
                ...
                .map(tool => (<div key={tool.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-primary/10 transition-colors cursor-pointer" onClick={() => handleToolSelect(tool)}>
              ...
              <tabs_1.TabsContent value="test" className="space-y-6">
                {selectedTool ? (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <card_1.Card className="glass-card">
                      <card_1.CardHeader>
                        <card_1.CardTitle className="flex items-center gap-2">
                          {selectedTool.icon}
                      ...
                      <card_1.CardContent className="space-y-4">
                        {selectedTool.parameters.map(param => (<div key={param.name} className="space-y-2">
                            <label className="text-sm font-medium">
                              {param.name}
                              {param.required && <span className="text-red-500 ml-1">*</span>}
                            </label>
                            {param.type === 'string' && param.options ? (<select_1.Select value={testParams[param.name] || ''} onValueChange={(value) => setTestParams(prev => ({ ...prev, [param.name]: value }))}>
                                <select_1.SelectTrigger>
                                  <select_1.SelectValue placeholder={`Select ${param.name}`}/>
                                </select_1.SelectTrigger>
                                <select_1.SelectContent>
                                  {param.options.map(option => (<select_1.SelectItem key={option} value={option}>
                                      {option}
                                    </select_1.SelectItem>))}
                                </select_1.SelectContent>
                              </select_1.Select>) : param.type === 'boolean' ? (<select_1.Select value={testParams[param.name]?.toString() || ''} onValueChange={(value) => setTestParams(prev => ({ ...prev, [param.name]: value === 'true' }))}>
                                <select_1.SelectTrigger>
                                  <select_1.SelectValue placeholder={`Select ${param.name}`}/>
                                </select_1.SelectTrigger>
                                <select_1.SelectContent>
                              ...
                              </select_1.Select>) : param.type === 'number' ? (<input_1.Input type="number" placeholder={`Enter ${param.name}`} value={testParams[param.name] || ''} onChange={(e) => setTestParams(prev => ({ ...prev, [param.name]: Number(e.target.value) }))}/>) : param.name === 'data' ? (<textarea_1.Textarea placeholder="Enter data as JSON array, e.g., [1, 2, 3, 4, 5]" value={testParams[param.name] || ''} onChange={(e) => {
                        try {
                    ...
                    }}/>) : (<input_1.Input placeholder={`Enter ${param.name}`} value={testParams[param.name] || ''} onChange={(e) => setTestParams(prev => ({ ...prev, [param.name]: e.target.value }))}/>)}
                            <p className="text-xs text-muted-foreground">
                              {param.description}
                            </p>
                          </div>))}

                        <button_1.Button onClick={handleTestRun} disabled={isLoading} className="w-full gradient-cyber-primary hover:gradient-cyber-secondary">
                          {isLoading ? (<>
                    ...
                    <card_1.Card className="glass-card">
                      <card_1.CardHeader>
                        <card_1.CardTitle className="flex items-center gap-2">
                          {testResults?.success ? (<lucide_react_1.CheckCircle className="w-5 h-5 text-green-500"/>) : testResults ? (<lucide_react_1.XCircle className="w-5 h-5 text-red-500"/>) : (<lucide_react_1.Info className="w-5 h-5 text-muted-foreground"/>)}
                          Test Results
                        </card_1.CardTitle>
                      </card_1.CardHeader>
                      <card_1.CardContent>
                        {testResults ? (<div className="space-y-4">
                            <div className="flex items-center gap-2">
                              <badge_1.Badge variant={testResults.success ? "default" : "destructive"}>
                            ...
                            {testResults.suggestions && (<div>
                                <h4 className="font-medium mb-2">Suggestions:</h4>
                                <ul className="list-disc list-inside space-y-1 text-sm">
                                  {testResults.suggestions.map((suggestion, index) => (<li key={index}>{suggestion}</li>))}
                                </ul>
                              </div>)}
                ...
                <card_1.Card className="glass-card">
                  ...
                  <card_1.CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {mcpTools.map(tool => (<div key={tool.id} className="space-y-2">
                          <h4 className="font-medium flex items-center gap-2">
                            {tool.icon}
                            {tool.name}
                          </h4>
                          <div className="space-y-1">
                            {tool.examples.slice(0, 2).map((example, index) => (<button_1.Button key={index} variant="outline" size="sm" className="w-full justify-start text-left h-auto p-2" onClick={() => {
                    setSelectedTool(tool);
                ...
                <card_1.Card className="glass-card">
                  ...
                  <card_1.CardContent className="space-y-6">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {mcpTools.map(tool => (<div key={tool.id} className="p-4 border rounded-lg">
                            <div className="flex items-center gap-2 mb-2">
                              {tool.icon}
                              <h4 className="font-medium">{tool.name}</h4>
                              <badge_1.Badge variant="outline">{tool.category}</badge_1.Badge>
                            </div>
