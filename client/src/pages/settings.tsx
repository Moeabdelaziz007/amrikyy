...
function SettingsPage() {
  ...
  const { data: me, isLoading: loadingMe } = (0, react_query_1.useQuery)({
    queryKey: ['me'],
    queryFn: () =>
      automation_api_1.automationApi.getMe().then((r: any) => r.data),
    onSuccess: (data: any) => {
      setName(data?.name || '');
      setEmail(data?.email || '');
  ...
  const updateProfile = (0, react_query_1.useMutation)({
    mutationFn: (payload: any) =>
      automation_api_1.automationApi.updateMe(payload).then((r: any) => r.data),
    onSuccess: () => {
  ...
  const updatePrefs = (0, react_query_1.useMutation)({
    mutationFn: (payload: any) =>
      automation_api_1.automationApi
              ...
              <card_1.CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4 items-center">
                ...
                <div>
                  ...
                  <select_1.Select value={language} onValueChange={setLanguage}>
                    ...
                    <select_1.SelectContent>
                      <select_1.SelectItem value="en">
                        English
                      </select_1.SelectItem>
                      <select_1.SelectItem value="ar">
                        العربية
                      </select_1.SelectItem>
                    </select_1.SelectContent>
                  </select_1.Select>
                ...
                <div className="md:col-span-2 text-right">
                  ...
                  >
                    {updatePrefs.isLoading ? 'Saving...' : 'Save Preferences'}
                  </button_1.Button>
                </div>
              </card_1.CardContent>
            </card_1.Card>
