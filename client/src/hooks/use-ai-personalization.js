...
function useAIPersonalization() {
    ...
    const analyzeBehavior = (0, react_1.useCallback)(async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const userProfile = await ai_personalization_1.AIPersonalizationEngine.analyzeUserBehavior(user.uid);
    ...
    const learnFromFeedback = (0, react_1.useCallback)(async (itemId, feedback, context) => {
        if (!user)
            return;
        try {
            await ai_personalization_1.AIPersonalizationEngine.learnFromFeedback(user.uid, itemId, feedback, context);
...
function usePersonalizedRecommendations(type) {
    ...
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    const loadRecommendations = (0, react_1.useCallback)(async (recommendationType = 'content', limit = 10) => {
        if (!user)
            return;
        try {
            setLoading(true);
            setError(null);
            const recs = await ai_personalization_1.AIPersonalizationEngine.generateRecommendations(user.uid, recommendationType, limit);
...
function usePersonalizationInsights() {
    ...
    const loadInsights = (0, react_1.useCallback)(async () => {
        ...
        try {
            setLoading(true);
            setError(null);
            const userInsights = await ai_personalization_1.AIPersonalizationEngine.generateInsights(user.uid);
...
function usePersonalizedTracking() {
    ...
    const trackInteraction = (0, react_1.useCallback)(async (itemId, action, context) => {
        if (!user)
            return;
        try {
    ...
    const trackContentEngagement = (0, react_1.useCallback)(async (contentId, engagementType, metadata) => {
    ...
    // Track feature usage
    const trackFeatureUsage = (0, react_1.useCallback)(async (featureId, usageType, metadata) => {
        await trackInteraction(featureId, usageType, metadata);
    }, [trackInteraction]);
    // Track workflow interaction
    const trackWorkflowInteraction = (0, react_1.useCallback)(async (workflowId, interactionType, metadata) => {
...
function useSmartSuggestions() {
    ...
    const getContextualSuggestions = (0, react_1.useCallback)((context) => {
        return recommendations.filter(rec => rec.metadata?.context === context ||
            rec.category === context);
    }, [recommendations]);
    // Track suggestion interaction
    const trackSuggestionInteraction = (0, react_1.useCallback)(async (suggestionId, action, metadata) => {
...
function useAdaptiveUI() {
    ...
    const getThemeConfig = (0, react_1.useCallback)(() => {
        ...
        const nightUsage = profile.behaviorPatterns.some(p => p.timeOfDay === 'night' && p.confidence > 0.7);
        if (nightUsage) {
