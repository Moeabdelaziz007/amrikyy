...
function usePosts(userId, options = {}) {
    ...
    const loadPosts = (0, react_1.useCallback)(async (reset = false) => {
        try {
            setLoading(true);
            setError(null);
            const result = await firebase_1.FirestoreService.getPosts(userId, options.limit || 10, reset ? null : lastDoc);
            if (reset) {
...
function usePostsRealtime(userId) {
    ...
    (0, react_1.useEffect)(() => {
        if (!userId)
            return;
        setLoading(true);
        const unsubscribe = firebase_1.FirestoreService.getPostsRealtime(userId, (newPosts) => {
            setPosts(newPosts);
            setLoading(false);
            setError(null);
        });
...
function useWorkflows(userId) {
    ...
    return {
        ...
        incrementWorkflowExecutions: firebase_1.FirestoreService.incrementWorkflowExecutions
    };
}
/**
...
function useAgents(userId) {
    ...
    return {
        ...
        incrementAgentInteractions: firebase_1.FirestoreService.incrementAgentInteractions
    };
}
/**
...
function useChatMessages(userId) {
    ...
    (0, react_1.useEffect)(() => {
        if (!userId)
            return;
        setLoading(true);
        const unsubscribe = firebase_1.FirestoreService.getChatMessagesRealtime(userId, (newMessages) => {
...
function usePostSearch(options = {}) {
    ...
    const [error, setError] = (0, react_1.useState)(null);
    const search = (0, react_1.useCallback)(async (searchTerm) => {
        try {
            setLoading(true);
            setError(null);
            const data = await firebase_1.FirestoreService.searchPosts(searchTerm, options.userId);
            setPosts(data);
        }
        catch (error) {
            setError(error.message);
...
function useBatchOperations() {
    const [loading, setLoading] = (0, react_1.useState)(false);
    const [error, setError] = (0, react_1.useState)(null);
    const batchCreatePosts = (0, react_1.useCallback)(async (userId, postsData) => {
        try {
            setLoading(true);
            setError(null);
            const result = await firebase_1.FirestoreService.batchCreatePosts(userId, postsData);
            return result;
        }
        catch (error) {
            setError(error.message);
