...
function PromptLibraryApp() {
    ...
    const [selectedDifficulty, setSelectedDifficulty] = (0, react_1.useState)("all");
    ...
    const [customCategory, setCustomCategory] = (0, react_1.useState)("Development");
    ...
    const handleAddToFavorites = (promptId) => {
        setFavorites(prev => prev.includes(promptId)
    ...
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        <tabs_1.TabsContent value="browse" className="space-y-6">
          <div className="flex gap-4">
            <input_1.Input placeholder="Search prompts..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1"/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPrompts.map(prompt => (<card_1.Card key={prompt.id} className="glass-card cursor-pointer hover:scale-105 transition-all" onClick={() => setSelectedPrompt(prompt)}>
                <card_1.CardHeader>
                ...
                handleAddToFavorites(prompt.id);
                      ...
                      <lucide_react_1.Heart className={`w-4 h-4 ${favorites.includes(prompt.id) ? 'text-red-500 fill-red-500' : ''}`}/>
                    </button_1.Button>
                  </card_1.CardTitle>
                ...
                <card_1.CardContent className="space-y-4">
                  <div className="flex items-center gap-2">
                    <badge_1.Badge variant="outline">{prompt.category}</badge_1.Badge>
                    <badge_1.Badge variant="secondary" className={getDifficultyColor(prompt.difficulty)}>
                      {prompt.difficulty}
                    </badge_1.Badge>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    ...
                    {prompt.tags.length > 3 && (<badge_1.Badge variant="outline" className="text-xs">
        ...
        <tabs_1.TabsContent value="favorites" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {prompts.filter(p => favorites.includes(p.id)).map(prompt => (<card_1.Card key={prompt.id} className="glass-card">
                <card_1.CardHeader>
                  <card_1.CardTitle className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                    ...
                    <button_1.Button size="sm" variant="ghost" onClick={() => handleAddToFavorites(prompt.id)}>
                      <lucide_react_1.Heart className="w-4 h-4 text-red-500 fill-red-500"/>
                    </button_1.Button>
                  </card_1.CardTitle>
                  <p className="text-sm text-muted-foreground">
                ...
                <card_1.CardContent>
                  <div className="space-y-2">
                    ...
                    <button_1.Button className="w-full" onClick={() => handleUsePrompt(prompt)}>
                      <lucide_react_1.Play className="w-4 h-4 mr-2"/>
                      Use Prompt
                    </button_1.Button>
                  </div>
          ...
          <card_1.Card className="glass-card">
            ...
            <card_1.CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <input_1.Input placeholder="Enter prompt title" value={customTitle} onChange={(e) => setCustomTitle(e.target.value)}/>
              </div>

              <div className="space-y-2">
                <label className="text-sm font-medium">Category</label>
                <select_1.Select value={customCategory} onValueChange={setCustomCategory}>
                  <select_1.SelectTrigger>
                    <select_1.SelectValue />
                  </select_1.SelectTrigger>
                  <select_1.SelectContent>
                    {promptCategories.map(category => (<select_1.SelectItem key={category} value={category}>
                        {category}
                      </select_1.SelectItem>))}
                  </select_1.SelectContent>
                </select_1.Select>
              ...
              <div className="space-y-2">
                ...
                <textarea_1.Textarea placeholder="Enter your custom prompt..." value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} rows={8}/>
              </div>

              <button_1.Button onClick={handleAddCustomPrompt} disabled={!customPrompt.trim() || !customTitle.trim()} className="w-full">
      ...
      {selectedPrompt && (<card_1.Card className="glass-card">
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center justify-between">
              {selectedPrompt.title}
              <button_1.Button size="sm" variant="ghost" onClick={() => setSelectedPrompt(null)}>
          ...
          <card_1.CardContent className="space-y-4">
            <div className="flex items-center gap-2">
              <badge_1.Badge variant="outline">{selectedPrompt.category}</badge_1.Badge>
              <badge_1.Badge variant="secondary" className={getDifficultyColor(selectedPrompt.difficulty)}>
                {selectedPrompt.difficulty}
              </badge_1.Badge>
              <badge_1.Badge variant="outline">{selectedPrompt.author}</badge_1.Badge>
            </div>
            ...
            {selectedPrompt.examples && (<div>
                <h4 className="font-medium mb-2">Examples:</h4>
                <ul className="list-disc list-inside space-y-1 text-sm">
                  {selectedPrompt.examples.map((example, index) => (<li key={index}>{example}</li>))}
                </ul>
              </div>)}
            ...
            <div className="flex gap-2">
              ...
              <button_1.Button variant="outline" onClick={() => handleAddToFavorites(selectedPrompt.id)}>
                <lucide_react_1.Heart className={`w-4 h-4 mr-2 ${favorites.includes(selectedPrompt.id) ? 'text-red-500 fill-red-500' : ''}`}/>
