...
const availablePlugins = [
    { id: 'cursor_cli', name: 'Cursor CLI', description: 'AI-powered code analysis and generation', category: 'Development', icon: <lucide_react_1.Bot className="w-4 h-4"/> },
    { id: 'comet_chrome', name: 'Comet Chrome', description: 'Web browsing and content analysis', category: 'Web', icon: <lucide_react_1.Brain className="w-4 h-4"/> },
    { id: 'web_scraper', name: 'Web Scraper', description: 'Extract content from web pages', category: 'Web', icon: <lucide_react_1.Zap className="w-4 h-4"/> },
    { id: 'data_analyzer', name: 'Data Analyzer', description: 'Statistical analysis and insights', category: 'Analytics', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'text_processor', name: 'Text Processor', description: 'NLP operations and text analysis', category: 'Text', icon: <lucide_react_1.Plug className="w-4 h-4"/> },
    { id: 'ai_generation_tool', name: 'AI Generator', description: 'Content and code generation', category: 'AI', icon: <lucide_react_1.Brain className="w-4 h-4"/> },
    { id: 'file_operations', name: 'File Operations', description: 'File system interactions', category: 'System', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'image_processor', name: 'Image Processor', description: 'Image analysis and manipulation', category: 'Media', icon: <lucide_react_1.Zap className="w-4 h-4"/> },
    { id: 'database_operations', name: 'Database Operations', description: 'Database management', category: 'Data', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'api_tester', name: 'API Tester', description: 'HTTP request testing', category: 'Development', icon: <lucide_react_1.Bot className="w-4 h-4"/> },
    { id: 'code_generator', name: 'Code Generator', description: 'Template-based code generation', category: 'Development', icon: <lucide_react_1.Bot className="w-4 h-4"/> },
    { id: 'data_visualizer', name: 'Data Visualizer', description: 'Chart and graph creation', category: 'Analytics', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'automation', name: 'Automation', description: 'Task automation and scheduling', category: 'System', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'knowledge_base', name: 'Knowledge Base', description: 'Information retrieval', category: 'AI', icon: <lucide_react_1.Brain className="w-4 h-4"/> },
    { id: 'system_info', name: 'System Info', description: 'System information gathering', category: 'System', icon: <lucide_react_1.Settings className="w-4 h-4"/> },
    { id: 'code_formatter', name: 'Code Formatter', description: 'Code formatting and styling', category: 'Development', icon: <lucide_react_1.Bot className="w-4 h-4"/> }
];
    ...
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <tabs_1.TabsList className="grid w-full grid-cols-3">
          ...
          <tabs_1.TabsTrigger value="history">Execution History</tabs_1.TabsTrigger>
        </tabs_1.TabsList>

        <tabs_1.TabsContent value="agents" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {availableAgents.map(agent => (<card_1.Card key={agent.id} className={`glass-card cursor-pointer transition-all hover:scale-105 ${selectedAgent?.id === agent.id ? 'ring-2 ring-primary' : ''}`} onClick={() => setSelectedAgent(agent)}>
                <card_1.CardHeader>
                ...
                <card_1.CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {agent.capabilities.map(capability => (<badge_1.Badge key={capability} variant="outline" className="text-xs">
                          {capability}
                        </badge_1.Badge>))}
                    </div>
                  </div>
                ...
                return plugin ? (<badge_1.Badge key={pluginId} variant="secondary" className="text-xs">
                            {plugin.name}
                          </badge_1.Badge>) : null;
            })}
                      {agent.plugins.length > 4 && (<badge_1.Badge variant="secondary" className="text-xs">
        ...
        <tabs_1.TabsContent value="execute" className="space-y-6">
          {selectedAgent ? (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <card_1.Card className="glass-card">
                ...
                <card_1.CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <textarea_1.Textarea placeholder="Describe the complex task you want the agent to perform..." value={task} onChange={(e) => setTask(e.target.value)} rows={6}/>
                  </div>
                ...
                const plugin = availablePlugins.find(p => p.id === pluginId);
                return plugin ? (<badge_1.Badge key={pluginId} variant="outline" className="text-xs">
                  ...
                  <button_1.Button onClick={handleAgentExecution} disabled={isExecuting || !task.trim()} className="w-full gradient-cyber-primary hover:gradient-cyber-secondary">
                    {isExecuting ? (<>
                ...
                <card_1.CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <ul className="list-disc list-inside space-y-1 text-sm">
                      {selectedAgent.capabilities.map(capability => (<li key={capability}>{capability}</li>))}
                    </ul>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      {selectedAgent.plugins.map(pluginId => {
                const plugin = availablePlugins.find(p => p.id === pluginId);
                return plugin ? (<div key={pluginId} className="flex items-center gap-2 text-sm">
                            {plugin.icon}
                            <span>{plugin.name}</span>
                            <badge_1.Badge variant="outline" className="text-xs">{plugin.category}</badge_1.Badge>
                          </div>) : null;
            })}
                    </div>
                  </div>
            ...
            </div>) : (<card_1.Card className="glass-card">
        ...
        <tabs_1.TabsContent value="history" className="space-y-6">
          <div className="space-y-4">
            {executions.length === 0 ? (<card_1.Card className="glass-card">
                <card_1.CardContent className="text-center py-12">
                  <lucide_react_1.Bot className="w-12 h-12 mx-auto mb-4 text-muted-foreground opacity-50"/>
              ...
              </card_1.Card>) : (executions.map(execution => {
            const agent = availableAgents.find(a => a.id === execution.agentId);
            return (<card_1.Card key={execution.id} className="glass-card">
                    <card_1.CardHeader>
                      <card_1.CardTitle className="flex items-center gap-2">
                        {agent?.icon} {agent?.name}
                        <badge_1.Badge variant={execution.status === 'completed' ? 'default' : execution.status === 'failed' ? 'destructive' : 'secondary'}>
                          {execution.status}
                        </badge_1.Badge>
                      </card_1.CardTitle>
                      <p className="text-sm text-muted-foreground">
                    ...
                    <card_1.CardContent>
                      <div className="space-y-4">
                        ...
                        {execution.steps.length > 0 && (<div>
                            <h4 className="font-medium mb-2">Execution Steps:</h4>
                            <div className="space-y-2">
                              {execution.steps.map((step, index) => (<div key={step.id} className="flex items-center gap-2 p-2 rounded bg-muted/50">
                                  ...
                                  <badge_1.Badge variant={step.status === 'completed' ? 'default' : 'secondary'} className="text-xs">
                                    {step.status}
                                  </badge_1.Badge>
                                </div>))}
                            </div>
