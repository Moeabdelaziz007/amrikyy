...
function PluginManagerApp() {
    ...
    const handleUpdatePlugin = (pluginId, updates) => {
        setPlugins(prev => prev.map(plugin => plugin.id === pluginId ? { ...plugin, ...updates } : plugin));
    };
    ...
    const getStatusIcon = (status) => {
        switch (status) {
            case 'active': return <lucide_react_1.CheckCircle className="w-4 h-4 text-green-500"/>;
            case 'inactive': return <lucide_react_1.XCircle className="w-4 h-4 text-yellow-500"/>;
            case 'error': return <lucide_react_1.XCircle className="w-4 h-4 text-red-500"/>;
            default: return <lucide_react_1.XCircle className="w-4 h-4 text-gray-500"/>;
        }
    };
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <tabs_1.TabsList className="grid w-full grid-cols-4">
          ...
          <tabs_1.TabsTrigger value="configs">Configurations</tabs_1.TabsTrigger>
        ...
        <tabs_1.TabsContent value="overview" className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            return (<div key={category} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                        <div>
        ...
        <tabs_1.TabsContent value="plugins" className="space-y-6">
          <div className="flex gap-4">
            <input_1.Input placeholder="Search plugins..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="flex-1"/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredPlugins.map(plugin => (<card_1.Card key={plugin.id} className="glass-card">
                <card_1.CardHeader>
                ...
                <card_1.CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {plugin.capabilities.map(capability => (<badge_1.Badge key={capability} variant="secondary" className="text-xs">
                          {capability}
                        </badge_1.Badge>))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {plugin.dependencies.map(dep => (<badge_1.Badge key={dep} variant="outline" className="text-xs">
                          {dep}
                        </badge_1.Badge>))}
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button_1.Button size="sm" variant={plugin.status === 'active' ? 'destructive' : 'default'} onClick={() => handleTogglePlugin(plugin.id)} className="flex-1">
                      {plugin.status === 'active' ? 'Deactivate' : 'Activate'}
                    </button_1.Button>
                    <button_1.Button size="sm" variant="outline" onClick={() => handleCreateConfig(plugin.id)}>
        ...
        <tabs_1.TabsContent value="configs" className="space-y-6">
          ...
          {configs.length === 0 ? (<card_1.Card className="glass-card">
              ...
              {configs.map(config => {
                ...
                return (<card_1.Card key={config.id} className="glass-card">
                    <card_1.CardHeader>
                      <card_1.CardTitle className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          {plugin?.icon}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <button_1.Button size="sm" variant="ghost" onClick={() => handleDeleteConfig(config.id)}>
        ...
        <tabs_1.TabsContent value="dependencies" className="space-y-6">
          <card_1.Card className="glass-card">
            ...
            return (<div key={dep} className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                        <div>
                        ...
                        <badge_1.Badge variant={activePlugins.length > 0 ? 'default' : 'secondary'}>
