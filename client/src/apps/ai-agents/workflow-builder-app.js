...
function WorkflowBuilderApp() {
    ...
    const updateStep = (stepId, updates) => {
        ...
        const updatedWorkflow = {
            ...selectedWorkflow,
            steps: selectedWorkflow.steps.map(step => step.id === stepId ? { ...step, ...updates } : step)
        };
        setSelectedWorkflow(updatedWorkflow);
        setWorkflows(prev => prev.map(w => w.id === selectedWorkflow.id ? updatedWorkflow : w));
    };
    const deleteStep = (stepId) => {
        ...
        const updatedWorkflow = {
            ...
            connections: selectedWorkflow.connections.filter(conn => conn.from !== stepId && conn.to !== stepId)
        };
        setSelectedWorkflow(updatedWorkflow);
        setWorkflows(prev => prev.map(w => w.id === selectedWorkflow.id ? updatedWorkflow : w));
    };
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          {selectedWorkflow && (<>
              ...
              <button_1.Button onClick={executeWorkflow} disabled={isExecuting} className="gradient-cyber-primary hover:gradient-cyber-secondary">
                {isExecuting ? (<>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <tabs_1.TabsList className="grid w-full grid-cols-3">
          <tabs_1.TabsTrigger value="builder">Workflow Builder</tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="library">Workflow Library</tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="executions">Executions</tabs_1.TabsTrigger>
        </tabs_1.TabsList>

        <tabs_1.TabsContent value="builder" className="space-y-6">
          {selectedWorkflow ? (<div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
              <div className="lg:col-span-1 space-y-4">
                <card_1.Card className="glass-card">
                  ...
                  <card_1.CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      <input_1.Input value={selectedWorkflow.name} onChange={(e) => setSelectedWorkflow({
                ...selectedWorkflow,
                name: e.target.value
            })}/>
                    </div>
                    <div className="space-y-2">
                      ...
                      <textarea_1.Textarea value={selectedWorkflow.description} onChange={(e) => setSelectedWorkflow({
                ...selectedWorkflow,
                description: e.target.value
                    ...
                    <div className="space-y-2">
                      ...
                      <select_1.Select value={selectedWorkflow.status} onValueChange={(value) => setSelectedWorkflow({
                ...selectedWorkflow,
                status: value
            })}>
                ...
                <card_1.Card className="glass-card">
                  ...
                  <card_1.CardContent className="space-y-2">
                    <button_1.Button onClick={() => addStep('agent')} variant="outline" className="w-full justify-start">
                      <lucide_react_1.Bot className="w-4 h-4 mr-2"/>
                      AI Agent
                    </button_1.Button>
                    <button_1.Button onClick={() => addStep('plugin')} variant="outline" className="w-full justify-start">
                      <lucide_react_1.Zap className="w-4 h-4 mr-2"/>
                      MCP Plugin
                    </button_1.Button>
                    <button_1.Button onClick={() => addStep('condition')} variant="outline" className="w-full justify-start">
                      <lucide_react_1.Workflow className="w-4 h-4 mr-2"/>
                      Condition
                    </button_1.Button>
                    <button_1.Button onClick={() => addStep('delay')} variant="outline" className="w-full justify-start">
                ...
                <card_1.Card className="glass-card">
                  ...
                  <card_1.CardContent>
                    <div className="min-h-[600px] border-2 border-dashed border-border rounded-lg p-6">
                      {selectedWorkflow.steps.length === 0 ? (<div className="flex items-center justify-center h-full">
                          ...
                          {selectedWorkflow.steps.map((step, index) => (<div key={step.id} className="flex items-center gap-4">
                              <card_1.Card className="flex-1">
                                <card_1.CardContent className="p-4">
                                  <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2">
                                      {step.type === 'agent' && <lucide_react_1.Bot className="w-4 h-4"/>}
                                      {step.type === 'plugin' && <lucide_react_1.Zap className="w-4 h-4"/>}
                                      {step.type === 'condition' && <lucide_react_1.Workflow className="w-4 h-4"/>}
                                      {step.type === 'delay' && <lucide_react_1.ArrowRight className="w-4 h-4"/>}
                                      <span className="font-medium">{step.name}</span>
                                      <badge_1.Badge variant="outline">{step.type}</badge_1.Badge>
                                    </div>
                                    <button_1.Button size="sm" variant="ghost" onClick={() => deleteStep(step.id)}>
                                      <lucide_react_1.Trash2 className="w-4 h-4"/>
                                    </button_1.Button>
                                  </div>
                                  
                                  {step.type === 'agent' && (<div className="mt-2">
                                      <select_1.Select value={step.config.agentId || ''} onValueChange={(value) => updateStep(step.id, {
                            config: { ...step.config, agentId: value }
                                        ...
                                        <select_1.SelectContent>
                                          {availableAgents.map(agent => (<select_1.SelectItem key={agent.id} value={agent.id}>
                                  ...
                                  {step.type === 'plugin' && (<div className="mt-2">
                                      <select_1.Select value={step.config.pluginId || ''} onValueChange={(value) => updateStep(step.id, {
                            config: { ...step.config, pluginId: value }
                                        ...
                                        <select_1.SelectContent>
                                          {availablePlugins.map(plugin => (<select_1.SelectItem key={plugin.id} value={plugin.id}>
                                  ...
                                  {step.type === 'condition' && (<div className="mt-2">
                                      <input_1.Input placeholder="Condition expression" value={step.config.condition || ''} onChange={(e) => updateStep(step.id, {
                            config: { ...step.config, condition: e.target.value }
                        })}/>
                                    </div>)}

                                  {step.type === 'delay' && (<div className="mt-2">
                                      <input_1.Input type="number" placeholder="Delay in seconds" value={step.config.delay || ''} onChange={(e) => updateStep(step.id, {
                            config: { ...step.config, delay: Number(e.target.value) }
                              ...
                              {index < selectedWorkflow.steps.length - 1 && (<div className="flex flex-col items-center">
        ...
        <tabs_1.TabsContent value="library" className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {workflows.map(workflow => (<card_1.Card key={workflow.id} className="glass-card cursor-pointer hover:scale-105 transition-all" onClick={() => setSelectedWorkflow(workflow)}>
                <card_1.CardHeader>
                  <card_1.CardTitle className="flex items-center justify-between">
                    {workflow.name}
                    <badge_1.Badge variant={workflow.status === 'active' ? 'default' : 'secondary'}>
                      {workflow.status}
                    </badge_1.Badge>
                  </card_1.CardTitle>
                ...
                <card_1.CardContent>
                  <div className="space-y-2">
                    ...
                    {workflow.lastRun && (<div className="text-sm">
