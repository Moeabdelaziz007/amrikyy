...
function CometChromeApp({ onExecute }) {
    ...
    return (<div className="space-y-6">
      <card_1.Card className="glass-card">
        ...
        <card_1.CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <select_1.Select value={action} onValueChange={setAction}>
              ...
              <select_1.SelectContent>
                <select_1.SelectItem value="analyze_page">Analyze Page</select_1.SelectItem>
                <select_1.SelectItem value="extract_content">Extract Content</select_1.SelectItem>
                <select_1.SelectItem value="summarize_article">Summarize Article</select_1.SelectItem>
                <select_1.SelectItem value="find_similar">Find Similar</select_1.SelectItem>
                <select_1.SelectItem value="translate_content">Translate Content</select_1.SelectItem>
                <select_1.SelectItem value="generate_questions">Generate Questions</select_1.SelectItem>
                <select_1.SelectItem value="create_outline">Create Outline</select_1.SelectItem>
                <select_1.SelectItem value="extract_links">Extract Links</select_1.SelectItem>
                <select_1.SelectItem value="analyze_sentiment">Analyze Sentiment</select_1.SelectItem>
                <select_1.SelectItem value="get_keywords">Get Keywords</select_1.SelectItem>
              </select_1.SelectContent>
            </select_1.Select>
          </div>
          ...
          <div className="space-y-2">
            ...
            <input_1.Input placeholder="https://example.com" value={url} onChange={(e) => setUrl(e.target.value)}/>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Content (Optional)</label>
            <textarea_1.Textarea placeholder="Content to analyze..." value={content} onChange={(e) => setContent(e.target.value)} rows={4}/>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <input_1.Input type="number" placeholder="10" value={maxResults} onChange={(e) => setMaxResults(Number(e.target.value))}/>
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Context (Optional)</label>
            <textarea_1.Textarea placeholder="Additional context for the analysis" value={context} onChange={(e) => setContext(e.target.value)} rows={2}/>
          </div>

          <button_1.Button onClick={handleExecute} disabled={isLoading || (!url.trim() && !content.trim())} className="w-full gradient-cyber-primary hover:gradient-cyber-secondary">
            {isLoading ? (<>
      ...
      {result && (<card_1.Card className="glass-card">
          <card_1.CardHeader>
            <card_1.CardTitle className="flex items-center gap-2">
          ...
          <card_1.CardContent>
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <badge_1.Badge variant={result.success ? "default" : "destructive"}>
              ...
              {result.comet_features && (<div>
                  <h4 className="font-medium mb-2">Comet Features:</h4>
                  <div className="flex flex-wrap gap-2">
                    {result.comet_features.map((feature, index) => (<badge_1.Badge key={index} variant="outline">{feature}</badge_1.Badge>))}
                  </div>
                </div>)}
            </div>
          </card_1.CardContent>
