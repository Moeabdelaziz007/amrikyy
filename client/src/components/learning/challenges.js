...
function Challenges({ challenges, userProgress = [], onJoinChallenge, onViewDetails }) {
    const getChallengeTypeIcon = (type) => {
        switch (type) {
            ...
            default:
                return <lucide_react_1.Circle className="h-4 w-4 text-muted-foreground"/>;
        }
    };
    ...
    return (<div className="space-y-4">
      ...
      <div className="grid gap-4">
        {challenges.map((challenge) => {
            ...
            return (<card_1.Card key={challenge.id} className={`glass-card transition-all duration-200 ${isCompleted
              ...
              <card_1.CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      <card_1.CardTitle className="text-base flex items-center space-x-2">
                        ...
                        {isCompleted && (<lucide_react_1.CheckCircle className="h-4 w-4 text-green-500"/>)}
                      </card_1.CardTitle>
                  ...
                  <div className="flex flex-col items-end space-y-2">
                    <badge_1.Badge className={getChallengeTypeColor(challenge.type)}>
                      {challenge.type}
                    </badge_1.Badge>
              ...
              <card_1.CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {challenge.requirements.map((req, index) => (<div key={index} className="flex items-center justify-between text-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {challenge.rewards.points > 0 && (<badge_1.Badge variant="secondary" className="neon-glow-sm">
                        <lucide_react_1.Zap className="h-3 w-3 mr-1"/>
                        {challenge.rewards.points} points
                      </badge_1.Badge>)}
                    {challenge.rewards.badges.map((badgeId, index) => (<badge_1.Badge key={badgeId} variant="outline" className="neon-glow-sm">
                        <lucide_react_1.Award className="h-3 w-3 mr-1"/>
                        Badge {index + 1}
                      </badge_1.Badge>))}
                    {challenge.rewards.achievements.map((achievementId, index) => (<badge_1.Badge key={achievementId} variant="outline" className="neon-glow-sm">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex space-x-2">
                    {onViewDetails && (<button_1.Button variant="ghost" size="sm" onClick={() => onViewDetails(challenge.id)}>
                        Details
                      </button_1.Button>)}
                    {!isCompleted && onJoinChallenge && !challenge.participants.includes('current-user') && (<button_1.Button size="sm" onClick={() => onJoinChallenge(challenge.id)} className="neon-button">
                        Join Challenge
                      </button_1.Button>)}
                  </div>
                </div>
                ...
                {/* Completion Status */}
                {isCompleted && progress?.completedAt && (<div className="p-3 rounded-lg bg-green-500/10 border border-green-500/30">
        ...
        {challenges.length === 0 && (<card_1.Card className="glass-card">
