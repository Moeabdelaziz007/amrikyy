...
function Leaderboard({ entries, currentUserId, onUserClick }) {
    ...
    const getRankBadge = (index) => {
        switch (index) {
            case 0:
                return <badge_1.Badge className="bg-yellow-500/20 text-yellow-500 border-yellow-500/30">#1</badge_1.Badge>;
            case 1:
                return <badge_1.Badge className="bg-gray-500/20 text-gray-500 border-gray-500/30">#2</badge_1.Badge>;
            case 2:
                return <badge_1.Badge className="bg-amber-600/20 text-amber-600 border-amber-600/30">#3</badge_1.Badge>;
            default:
    ...
    return (<card_1.Card className="glass-card neon-glow-md">
      <card_1.CardHeader>
      ...
      <card_1.CardContent>
        <div className="space-y-3">
          {entries.map((entry, index) => (<div key={entry.userId} className={`flex items-center space-x-3 p-3 rounded-lg transition-all duration-200 ${entry.userId === currentUserId
                ? 'bg-primary/10 border border-primary/30 neon-glow-sm'
                : 'bg-secondary/30 hover:bg-secondary/50'} ${index < 3 ? 'border-l-4 border-l-primary' : ''}`} onClick={() => onUserClick?.(entry.userId)}>
              {/* Rank */}
              ...
              <avatar_1.Avatar className="h-10 w-10">
                <avatar_1.AvatarFallback className={getUserAvatar(entry.userId)}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center space-x-2">
                  ...
                  {entry.userId === currentUserId && (<badge_1.Badge variant="secondary" className="text-xs">
                      You
                    </badge_1.Badge>)}
                </div>
        ...
        {/* Additional Stats */}
        {entries.length > 0 && (<div className="mt-6 pt-4 border-t border-border/50">
