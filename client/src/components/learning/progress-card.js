...
function ProgressCard({ progress, onViewDetails, compact = false }) {
    ...
    if (compact) {
        return (<card_1.Card className="glass-card neon-glow-sm">
    ...
    return (<card_1.Card className="glass-card neon-glow-md">
      <card_1.CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          {onViewDetails && (<button_1.Button variant="ghost" size="sm" onClick={onViewDetails}>
              View Details
            </button_1.Button>)}
        </div>
      </card_1.CardHeader>
      ...
      <card_1.CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {progress.badges.slice(0, 6).map((badgeId, index) => (<badge_1.Badge key={badgeId} variant="secondary" className="neon-glow-sm text-xs">
                🏆 {index + 1}
              </badge_1.Badge>))}
            {progress.badges.length > 6 && (<badge_1.Badge variant="outline" className="text-xs">
        ...
        {/* Recent Achievements */}
        {progress.achievements.length > 0 && (<div className="space-y-2">
            <div className="flex items-center space-x-1">
              <lucide_react_1.Trophy className="h-4 w-4 text-yellow-500"/>
              <span className="text-sm font-medium">Recent Achievements</span>
            </div>
            <div className="space-y-2">
              {progress.achievements.slice(-3).map((achievement) => (<div key={achievement.id} className="flex items-center space-x-3 p-2 rounded-lg bg-secondary/30">
        ...
        <div className="space-y-2">
          ...
          <progress_1.Progress value={(progress.totalPoints % progress.weeklyGoal) / progress.weeklyGoal * 100} className="h-2"/>
        </div>
      </card_1.CardContent>
    </card_1.Card>);
}
