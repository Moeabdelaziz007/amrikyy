...
function AIPersonalizationDashboard({ userId }) {
  ...
  const [selectedRecommendationType, setSelectedRecommendationType] = (0,
  react_1.useState)('content');
  const {
    profile,
    loading: profileLoading,
    analyzeBehavior,
  } = (0, use_ai_personalization_1.useAIPersonalization)();
  const {
    recommendations,
    loading: recLoading,
    loadRecommendations,
  } = (0, use_ai_personalization_1.usePersonalizedRecommendations)(
    selectedRecommendationType
  );
  const { insights, loading: insightsLoading } = (0,
  use_ai_personalization_1.usePersonalizationInsights)();
  const { uiPreferences, getLayoutConfig, isAdaptive } = (0,
  use_ai_personalization_1.useAdaptiveUI)();
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <lucide_react_1.RefreshCw
              className={`h-4 w-4 mr-2 ${profileLoading ? 'animate-spin' : ''}`}
            />
            Refresh Analysis
          </button_1.Button>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          ...
          <p className="text-xs text-muted-foreground">Topics of interest</p>
        </card_1.CardContent>
      </card_1.Card>
      ...
      <card_1.Card className="col-span-1 md:col-span-2">
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>
            AI-detected personality characteristics
          </card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="space-y-4">
            {profile?.personalityTraits?.slice(0, 4).map((trait, index) => (
              ...
              <div key={insight.type} className="flex items-start space-x-3">
                <div
                  className={`w-2 h-2 rounded-full mt-2 ${
                    insight.type === 'usage_pattern'
                      ? 'bg-blue-500'
                      : insight.type === 'new_interest'
                        ? 'bg-green-500'
                        : insight.type === 'preference_change'
                          ? 'bg-yellow-500'
                          : 'bg-gray-500'
                  }`}
                />
                <div className="flex-1">
                  ...
                  {insight.actionable && (
                    <badge_1.Badge variant="outline" className="mt-1 text-xs">
                      Actionable
                    </badge_1.Badge>
                  )}
                </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recommendations.map((rec, index) => (
          <card_1.Card key={rec.id} className="relative">
            <card_1.CardHeader>
  ...
  return (
    <div className="space-y-6">
      {insights.map((insight, index) => (
        <card_1.Card key={insight.type} className="relative">
          <card_1.CardHeader>
            <div className="flex items-start justify-between">
    ...
    <div className="space-y-6">
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>
            Preferences learned from your behavior
          </card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="space-y-4">
            {profile?.preferences?.slice(0, 6).map((pref, index) => (
              <div
                key={pref.id}
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>
            Interface customizations based on your behavior
          </card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <badge_1.Badge
                    variant={
                      layoutConfig.sidebar.showAdvancedFeatures
                        ? 'default'
                        : 'secondary'
                    }
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <badge_1.Badge
                    variant={
                      layoutConfig.dashboard.showQuickActions
                        ? 'default'
                        : 'secondary'
                    }
                  >
                ...
                <div className="flex items-center justify-between">
                  ...
                  <badge_1.Badge
                    variant={
                      layoutConfig.dashboard.showSocialFeatures
                        ? 'default'
                        : 'secondary'
                    }
                  >
                ...
                {profile?.behaviorPatterns
                  ...
                  .map((pattern, index) => (
                    <div key={pattern.id} className="text-sm">
                      ...
                      <div className="text-xs text-muted-foreground">
                        Frequency: {pattern.frequency} • Confidence:{' '}
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>
            Topics and categories you're interested in
          </card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
