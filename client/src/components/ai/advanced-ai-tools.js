            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                <div className="text-2xl font-bold text-blue-500">
                  {Math.round((analytics.toolStats?.reduce((sum, tool) => sum + tool.usage.successRate, 0) / (analytics.toolStats?.length || 1)) * 100)}%
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                {toolsLoading ? (<div className="animate-pulse space-y-4">
                    {[...Array(5)].map((_, i) => (<div key={i} className="h-16 bg-muted rounded"></div>))}
                  </div>) : tools ? (tools.map(tool => (<div key={tool.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedTool?.id === tool.id ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'}`} onClick={() => handleToolSelect(tool)}>
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {tool.isActive ? (<badge_1.Badge variant="default" className="text-xs">Active</badge_1.Badge>) : (<badge_1.Badge variant="outline" className="text-xs">Inactive</badge_1.Badge>)}
                          </div>
                    ...
                    <alert_1.AlertDescription>No AI tools available</alert_1.AlertDescription>
                  </alert_1.Alert>)}
              </card_1.CardContent>
            </card_1.Card>
              ...
              <card_1.CardContent className="space-y-4">
                {selectedTool ? (<>
                    <div className="space-y-4">
                      {selectedTool.parameters.map(param => (<div key={param.name}>
                          <label_1.Label htmlFor={param.name} className="text-sm font-medium">
                            {param.name}
                            {param.required && <span className="text-red-500 ml-1">*</span>}
                          </label_1.Label>
                          <div className="mt-1">
                            {param.type === 'string' && param.name.includes('text') ? (<textarea_1.Textarea id={param.name} placeholder={param.description} value={toolParams[param.name] || ''} onChange={(e) => handleParamChange(param.name, e.target.value)} rows={3}/>) : param.type === 'boolean' ? (<select_1.Select value={toolParams[param.name]?.toString() || ''} onValueChange={(value) => handleParamChange(param.name, value === 'true')}>
                                <select_1.SelectTrigger>
                                  <select_1.SelectValue placeholder={param.description}/>
                                </select_1.SelectTrigger>
                                <select_1.SelectContent>
                                  <select_1.SelectItem value="true">True</select_1.SelectItem>
                                  <select_1.SelectItem value="false">False</select_1.SelectItem>
                                </select_1.SelectContent>
                              </select_1.Select>) : (<input_1.Input id={param.name} type={param.type === 'number' ? 'number' : 'text'} placeholder={param.description} value={toolParams[param.name] || ''} onChange={(e) => handleParamChange(param.name, param.type === 'number' ? Number(e.target.value) : e.target.value)}/>)}
                          </div>
                          <p className="text-xs text-muted-foreground mt-1">{param.description}</p>
                        </div>))}
                    </div>

                    <button_1.Button onClick={handleExecuteTool} disabled={executeToolMutation.isPending} className="w-full">
                      {executeToolMutation.isPending ? 'Executing...' : 'Execute Tool'}
                    </button_1.Button>

                    {executeToolMutation.isError && (<alert_1.Alert variant="destructive">
                        <alert_1.AlertDescription>{executeToolMutation.error?.message}</alert_1.AlertDescription>
                      </alert_1.Alert>)}

                    {executeToolMutation.isSuccess && executeToolMutation.data && (<div className="space-y-4">
                        ...
                        {executeToolMutation.data.suggestions && executeToolMutation.data.suggestions.length > 0 && (<div>
                            <span className="font-medium">Suggestions:</span>
                            <ul className="text-sm text-muted-foreground mt-1">
                              {executeToolMutation.data.suggestions.map((suggestion, index) => (<li key={index}>• {suggestion}</li>))}
                            </ul>
                          </div>)}
                      </div>)}
          ...
          <card_1.Card>
            ...
            <card_1.CardContent className="space-y-4">
              <div className="flex gap-4">
                <input_1.Input placeholder="Search for tools..." value={discoverToolsMutation.variables?.query || ''} onChange={(e) => {
            ...
            discoverToolsMutation.mutate({ query, category });
              ...
              {discoverToolsMutation.isSuccess && discoverToolsMutation.data && (<div className="space-y-4">
                  <h4 className="font-medium">Search Results:</h4>
                  {discoverToolsMutation.data.map((tool) => (<div key={tool.id} className="p-4 border rounded-lg">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex gap-2 mt-2">
                            {tool.capabilities.map(capability => (<badge_1.Badge key={capability} variant="outline" className="text-xs">
                                {capability}
                              </badge_1.Badge>))}
                          </div>
                        </div>
        ...
        <tabs_1.TabsContent value="analytics">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <card_1.Card>
              ...
              <card_1.CardContent>
                {analytics?.toolStats ? (<div className="space-y-4">
                    {analytics.toolStats.map((tool) => (<div key={tool.id} className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>{tool.name}</span>
                          <span>{Math.round(tool.usage.successRate * 100)}%</span>
                        </div>
                        <progress_1.Progress value={tool.usage.successRate * 100}/>
                      </div>))}
            ...
            <card_1.Card>
              ...
              <card_1.CardContent>
                {categories ? (<div className="space-y-4">
