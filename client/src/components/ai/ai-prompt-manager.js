...
function AIPromptManager() {
    ...
    const [showExecutionDialog, setShowExecutionDialog] = (0, react_1.useState)(false);
    ...
    const handleVariableChange = (0, react_1.useCallback)((variableName, value) => {
    ...
    const renderVariableInput = (variable) => {
        switch (variable.type) {
            case 'select':
                return (<select_1.Select value={promptVariables[variable.name] || variable.defaultValue} onValueChange={(value) => handleVariableChange(variable.name, value)}>
            <select_1.SelectTrigger>
              <select_1.SelectValue placeholder={variable.placeholder}/>
            </select_1.SelectTrigger>
            <select_1.SelectContent>
              {variable.options?.map(option => (<select_1.SelectItem key={option} value={option}>
                  {option}
                </select_1.SelectItem>))}
            </select_1.SelectContent>
          </select_1.Select>);
            ...
            {variable.options?.map(option => (<label key={option} className="flex items-center space-x-2">
                <input type="checkbox" checked={promptVariables[variable.name]?.includes(option) || false} onChange={(e) => {
            ...
            case 'textarea':
                return (<textarea_1.Textarea placeholder={variable.placeholder} value={promptVariables[variable.name] || variable.defaultValue || ''} onChange={(e) => handleVariableChange(variable.name, e.target.value)} rows={4}/>);
            case 'number':
                return (<input_1.Input type="number" placeholder={variable.placeholder} value={promptVariables[variable.name] || variable.defaultValue || ''} onChange={(e) => handleVariableChange(variable.name, Number(e.target.value))}/>);
            case 'boolean':
                return (<select_1.Select value={promptVariables[variable.name]?.toString() || variable.defaultValue?.toString() || 'false'} onValueChange={(value) => handleVariableChange(variable.name, value === 'true')}>
            <select_1.SelectTrigger>
            ...
            default:
                return (<input_1.Input placeholder={variable.placeholder} value={promptVariables[variable.name] || variable.defaultValue || ''} onChange={(e) => handleVariableChange(variable.name, e.target.value)}/>);
        }
    };
    return (<div className="space-y-6">
          ...
          <div className="flex gap-4 items-center">
            <div className="flex-1">
              <div className="relative">
                ...
                <input_1.Input placeholder="Search prompts..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10"/>
              </div>
            </div>
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <tabs_1.TabsContent value="prompts" className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <card_1.CardContent>
                  <scroll_area_1.ScrollArea className="h-96">
                    <div className="space-y-3">
                      {filteredPrompts.map((prompt) => (<card_1.Card key={prompt.id} className={`cursor-pointer transition-all hover:shadow-md ${selectedPrompt?.id === prompt.id ? 'ring-2 ring-primary' : ''}`} onClick={() => handlePromptSelect(prompt)}>
                          <card_1.CardContent className="p-4">
                            <div className="flex items-start justify-between mb-2">
                              <h3 className="font-medium text-sm">{prompt.title}</h3>
                              <badge_1.Badge className={`text-xs ${getComplexityColor(prompt.usage.complexity)}`}>
                            ...
                            <div className="flex flex-wrap gap-1 mt-2">
                              {prompt.tags.slice(0, 3).map(tag => (<badge_1.Badge key={tag} variant="outline" className="text-xs">
                                  {tag}
                                </badge_1.Badge>))}
                            </div>
                          </card_1.CardContent>
            ...
            <div className="lg:col-span-2">
              {selectedPrompt ? (<div className="space-y-4">
                  {/* Prompt Header */}
                  <card_1.Card>
                    <card_1.CardHeader>
                      <div className="flex items-start justify-between">
                        ...
                        <badge_1.Badge className={`${getComplexityColor(selectedPrompt.usage.complexity)} text-white`}>
                  ...
                  {selectedPrompt.variables && selectedPrompt.variables.length > 0 && (<card_1.Card>
                      <card_1.CardHeader>
                        <card_1.CardTitle>Configure Variables</card_1.CardTitle>
                      </card_1.CardHeader>
                      <card_1.CardContent className="space-y-4">
                        {selectedPrompt.variables.map((variable) => (<div key={variable.name} className="space-y-2">
                            <label_1.Label htmlFor={variable.name} className="flex items-center gap-2">
                  ...
                  <card_1.Card>
                    <card_1.CardContent className="p-6">
                      <button_1.Button onClick={executePrompt} disabled={isExecuting} className="w-full" size="lg">
                        {isExecuting ? (<>
                  ...
                  {/* Execution Result */}
                  {executionResult && (<card_1.Card>
                      <card_1.CardHeader>
                        <div className="flex items-center justify-between">
                          <card_1.CardTitle>Execution Result</card_1.CardTitle>
                          <button_1.Button variant="outline" size="sm" onClick={() => copyToClipboard(executionResult)}>
            ...
            {templates.map((template) => (<card_1.Card key={template.id}>
                ...
                <card_1.CardContent>
                  <div className="space-y-3">
                    ...
                    <div>
                      <label_1.Label className="text-sm font-medium">Includes Prompts</label_1.Label>
                      <div className="flex flex-wrap gap-1 mt-1">
                        {template.prompts.map(promptId => {
                const prompt = prompts.find(p => p.id === promptId);
                return prompt ? (<badge_1.Badge key={promptId} variant="secondary" className="text-xs">
                              {prompt.title}
                            </badge_1.Badge>) : null;
            })}
                      </div>
        ...
        <tabs_1.TabsContent value="analytics" className="space-y-4">
          ...
          <card_1.Card>
            ...
            .slice(0, 5)
            .map((prompt, index) => (<div key={prompt.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
      ...
      <dialog_1.Dialog open={showExecutionDialog} onOpenChange={setShowExecutionDialog}>
        <dialog_1.DialogContent className="max-w-4xl">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <button_1.Button variant="outline" onClick={() => setShowExecutionDialog(false)}>
                Close
              </button_1.Button>
            </div>
          </div>
