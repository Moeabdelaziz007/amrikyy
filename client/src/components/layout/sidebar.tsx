...
const navigation = [
  ...
  {
    name: 'Web Scraper',
    href: '/mcp-tools?tool=web_scraper',
    icon: 'fas fa-spider',
    category: 'mcp',
  },
  {
    name: 'Data Analyzer',
    href: '/mcp-tools?tool=data_analyzer',
    icon: 'fas fa-chart-line',
    category: 'mcp',
  },
  {
    name: 'Text Processor',
    href: '/mcp-tools?tool=text_processor',
    icon: 'fas fa-file-text',
    category: 'mcp',
  },
  {
    name: 'File Operations',
    href: '/mcp-tools?tool=file_operations',
    icon: 'fas fa-file',
    category: 'mcp',
  },
  {
    name: 'Image Processor',
    href: '/mcp-tools?tool=image_processor',
    icon: 'fas fa-image',
    category: 'mcp',
  },
  {
    name: 'Database Ops',
    href: '/mcp-tools?tool=database_operations',
    icon: 'fas fa-database',
    category: 'mcp',
  },
  {
    name: 'API Tester',
    href: '/mcp-tools?tool=api_tester',
    icon: 'fas fa-flask',
    category: 'mcp',
  },
  {
    name: 'Code Generator',
    href: '/mcp-tools?tool=code_generator',
    icon: 'fas fa-code',
    category: 'mcp',
  },
  {
    name: 'Data Visualizer',
    href: '/mcp-tools?tool=data_visualizer',
    icon: 'fas fa-chart-pie',
    category: 'mcp',
  },
  {
    name: 'Automation',
    href: '/mcp-tools?tool=automation',
    icon: 'fas fa-cogs',
    category: 'mcp',
  },
  {
    name: 'Knowledge Base',
    href: '/mcp-tools?tool=knowledge_base',
    icon: 'fas fa-book-open',
    category: 'mcp',
  },
  {
    name: 'System Info',
    href: '/mcp-tools?tool=system_info',
    icon: 'fas fa-info-circle',
    category: 'mcp',
  },
  {
    name: 'Code Formatter',
    href: '/mcp-tools?tool=code_formatter',
    icon: 'fas fa-indent',
    category: 'mcp',
  },
  {
    name: 'Cursor CLI',
    href: '/mcp-tools?tool=cursor_cli',
    icon: 'fas fa-terminal',
    category: 'mcp',
  },
  {
    name: 'Comet Chrome',
    href: '/mcp-tools?tool=comet_chrome',
    icon: 'fas fa-globe',
    category: 'mcp',
  },
  {
    name: 'Multilingual Assistant',
    href: '/mcp-tools?tool=multilingual_assistant',
    icon: 'fas fa-language',
    category: 'mcp',
  },
  {
    name: 'System Designer',
    href: '/mcp-tools?tool=system_designer',
    icon: 'fas fa-drafting-compass',
    category: 'mcp',
  },
  {
    name: 'Educational Tutor',
    href: '/mcp-tools?tool=educational_tutor',
    icon: 'fas fa-chalkboard-teacher',
    category: 'mcp',
  },
  {
    name: 'Wellness Coach',
    href: '/mcp-tools?tool=wellness_coach',
    icon: 'fas fa-heart',
    category: 'mcp',
  },
  {
...
function Sidebar() {
  ...
  const { data: userData } = (0, react_query_1.useQuery)({
    ...
    queryFn: () =>
      user?.uid
        ? Promise.resolve({ displayName: user.displayName, email: user.email })
        : null,
    enabled: !!user?.uid,
  });
      ...
      <div className="p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="font-bold text-xl neon-text animate-neon-flicker">
              AuraOS
            </span>
            ...
            return (
              <li key={item.name}>
                <wouter_2.Link href={item.href}>
                  ...
                  >
                    <i
                      className={`${item.icon} w-5 transition-transform duration-300 group-hover:scale-110`}
                    ></i>
            ...
            <>
              ...
              <li>
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-2 w-full text-left text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors"
                >
                  <i
                    className={`fas fa-chevron-${expandedSections.mcp ? 'down' : 'right'} w-4 transition-transform duration-200`}
                  ></i>
              ...
              {expandedSections.mcp &&
                groupedNavigation.mcp.map(item => {
                  const isActive = location === item.href;
                  return (
                    <li key={item.name} className="ml-4">
                      <wouter_2.Link href={item.href}>
                        <a
                          className={(0, utils_1.cn)(
                            'flex items-center gap-3 px-4 py-2 rounded-lg transition-all duration-300 group relative overflow-hidden text-sm',
                            isActive
                              ? 'gradient-cyber-primary text-white neon-glow-lg animate-neon-pulse'
                              : 'text-muted-foreground hover:text-foreground hover:bg-primary/10 hover:neon-glow-sm hover:border-primary/30 border border-transparent'
                          )}
                        ...
                        >
                          <i
                            className={`${item.icon} w-4 transition-transform duration-300 group-hover:scale-110`}
                          ></i>
                          <span className="font-medium">{item.name}</span>
                          {isActive && (
                            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-cyber-scan"></div>
        ...
        <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-primary/10 transition-all duration-300 group">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p
                className="text-sm font-medium text-foreground truncate neon-text"
