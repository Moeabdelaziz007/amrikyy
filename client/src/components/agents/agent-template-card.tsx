...
function AgentTemplateCard({ template, showActions = false }) {
  ...
  const useTemplateMutation = (0, react_query_1.useMutation)({
    mutationFn: async () => {
      await (0, queryClient_1.apiRequest)(
        'POST',
        `/api/agent-templates/${template.id}/use`
      );
    ...
    >
      <card_1.CardContent className="p-4">
        <div className="flex items-start gap-3">
          <div
            className={`w-8 h-8 bg-gradient-to-r ${getGradientFromCategory(template.category)} rounded-lg flex items-center justify-center flex-shrink-0`}
          >
          ...
          <div className="flex-1">
            <h4
              ...
              data-testid={`text-name-${template.id}`}
            >
              {template.name}
            </h4>
            <p
              className="text-sm text-muted-foreground mb-3"
              data-testid={`text-description-${template.id}`}
            >
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {template.isPopular && (
                <badge_1.Badge
                  ...
                  data-testid={`badge-popular-${template.id}`}
                >
                  Popular
                </badge_1.Badge>
              )}
            ...
            {showActions && (
              <div className="flex gap-2 mt-3">
                ...
                >
                  <i
                    className={`fas ${useTemplateMutation.isPending ? 'fa-spinner fa-spin' : 'fa-plus'} mr-1`}
                  ></i>
