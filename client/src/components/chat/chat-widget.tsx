...
function ChatWidget() {
  ...
  const {
    ...
    queryFn: () =>
      fetch('/api/chat/history?userId=user-1&limit=10').then(res => {
  ...
  const chatMutation = (0, react_query_1.useMutation)({
    mutationFn: async userMessage => {
      const response = await (0, queryClient_1.apiRequest)(
        'POST',
        '/api/chat',
        {
          message: userMessage,
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <div className="relative">
        ...
        >
          <i
            className={`fas ${isOpen ? 'fa-times' : 'fa-robot'} text-lg group-hover:scale-110 transition-transform`}
          ></i>
        </button_1.Button>

        {isOpen && (
          <card_1.Card
            className="absolute bottom-16 right-0 w-80 shadow-xl glass-card neon-glow-md"
              ...
              <scroll_area_1.ScrollArea className="h-60 p-4 cyber-scrollbar">
                <div className="space-y-4">
                  {messages.map(msg => (
                    ...
                    >
                      {msg.type === 'ai' && (
                        <avatar_1.Avatar className="w-6 h-6 flex-shrink-0">
                      ...
                      <div
                        className={`max-w-[80%] p-3 rounded-lg text-sm ${msg.type === 'user' ? 'bg-primary text-primary-foreground ml-auto' : 'bg-muted text-foreground'}`}
                      >
                        {msg.content}
                      </div>
                    </div>
                  ...
                  {chatMutation.isError && (
                    <div className="flex gap-3 justify-start">
                      <avatar_1.Avatar className="w-6 h-6 flex-shrink-0">
                        <avatar_1.AvatarFallback className="bg-destructive text-destructive-foreground">
