...
function WorkflowMarketplace({ userId }) {
    ...
    const { marketplace, loading: marketplaceLoading, searchTemplates } = (0, use_workflow_automation_1.useWorkflowMarketplace)(filters);
    const { recommendations, loading: recommendationsLoading } = (0, use_workflow_automation_1.useIntelligentWorkflowRecommendations)(6);
    ...
    return (<div className="space-y-6">
      ...
      <tabs_1.Tabs value={activeTab} onValueChange={setActiveTab}>
        <tabs_1.TabsList className="grid w-full grid-cols-3">
          <tabs_1.TabsTrigger value="marketplace">Marketplace</tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="recommendations">For You</tabs_1.TabsTrigger>
          <tabs_1.TabsTrigger value="trending">Trending</tabs_1.TabsTrigger>
        </tabs_1.TabsList>

        <tabs_1.TabsContent value="marketplace" className="space-y-6">
          <MarketplaceTab marketplace={marketplace} searchQuery={searchQuery} setSearchQuery={setSearchQuery} filters={filters} onFilterChange={handleFilterChange} viewMode={viewMode} setViewMode={setViewMode} sortBy={sortBy} setSortBy={setSortBy} onSearch={handleSearch}/>
        </tabs_1.TabsContent>

        <tabs_1.TabsContent value="recommendations" className="space-y-6">
          <RecommendationsTab recommendations={recommendations} loading={recommendationsLoading}/>
        </tabs_1.TabsContent>
...
function MarketplaceTab({ marketplace, searchQuery, setSearchQuery, filters, onFilterChange, viewMode, setViewMode, sortBy, setSortBy, onSearch }) {
    return (<div className="space-y-6">
      {/* Search and Filters */}
      <div className="flex items-center space-x-4">
        <div className="flex-1 relative">
          ...
          <input_1.Input placeholder="Search workflows..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10" onKeyPress={(e) => e.key === 'Enter' && onSearch()}/>
        </div>
      ...
      <div className="flex items-center space-x-2 overflow-x-auto pb-2">
        <button_1.Button variant={!filters.category ? "default" : "outline"} size="sm" onClick={() => onFilterChange({ category: undefined })}>
          All
        </button_1.Button>
        {marketplace?.categories?.map((category) => (<button_1.Button key={category.id} variant={filters.category === category.id ? "default" : "outline"} size="sm" onClick={() => onFilterChange({ category: category.id })} className="flex items-center gap-2">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="px-3 py-1 border rounded-md text-sm" aria-label="Sort workflows">
      ...
      <div className={viewMode === 'grid'
            ...
            : 'space-y-4'}>
        {(marketplace?.searchResults || marketplace?.templates || []).map((template) => (<WorkflowTemplateCard key={template.id} template={template} viewMode={viewMode}/>))}
      </div>
    </div>);
}
function WorkflowTemplateCard({ template, viewMode }) {
    const { createWorkflow } = (0, use_workflow_automation_1.useWorkflowTemplate)();
    ...
    if (viewMode === 'list') {
        return (<card_1.Card className="flex">
        <div className="flex-1 p-6">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <div className="flex items-center space-x-2 mb-2">
                <h3 className="text-lg font-semibold">{template.name}</h3>
                {template.isFeatured && (<badge_1.Badge variant="default" className="bg-gradient-to-r from-purple-500 to-pink-500">
                    Featured
                  </badge_1.Badge>)}
              </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <button_1.Button onClick={handleInstall} disabled={installing} size="sm">
    ...
    return (<card_1.Card className="group hover:shadow-lg transition-shadow">
      <card_1.CardHeader>
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-2">
      ...
      <card_1.CardContent>
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-1">
            {template.tags.slice(0, 3).map((tag) => (<badge_1.Badge key={tag} variant="outline" className="text-xs">
                {tag}
              </badge_1.Badge>))}
          </div>
          ...
          {/* Difficulty */}
          <div className="flex items-center justify-between">
            <badge_1.Badge variant={template.difficulty === 'beginner' ? 'default' :
          ...
          <div className="flex items-center space-x-2 pt-2">
            <button_1.Button onClick={handleInstall} disabled={installing} className="flex-1">
              <lucide_react_1.Download className="h-4 w-4 mr-2"/>
              {installing ? 'Installing...' : 'Install'}
            </button_1.Button>
            <button_1.Button variant="outline" size="sm">
...
function RecommendationsTab({ recommendations, loading }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recommendations.map((template) => (<WorkflowTemplateCard key={template.id} template={template} viewMode="grid"/>))}
      </div>
...
function TrendingTab({ marketplace }) {
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {marketplace?.trending?.map((template) => (<WorkflowTemplateCard key={template.id} template={template} viewMode="grid"/>))}
      </div>
    </div>);
}
