...
function WorkflowBuilder() {
    ...
    return (<card_1.Card className="h-[600px]">
      <card_1.CardHeader>
        <div className="flex items-center justify-between">
          <card_1.CardTitle>Workflow Builder</card_1.CardTitle>
          <div className="flex gap-2">
            <button_1.Button size="sm" variant="outline" data-testid="button-save-workflow">
        ...
        <div className="flex gap-6 h-full">
          ...
          <div className="w-48 space-y-3">
            ...
            <div className="space-y-2">
              <button_1.Button variant="outline" size="sm" className="w-full justify-start" onClick={() => addNode('trigger')} data-testid="button-add-trigger">
                <i className="fas fa-play-circle text-green-500 mr-2"></i>
                Trigger
              </button_1.Button>
              
              <button_1.Button variant="outline" size="sm" className="w-full justify-start" onClick={() => addNode('ai')} data-testid="button-add-ai">
                <i className="fas fa-robot text-primary mr-2"></i>
                AI Process
              </button_1.Button>
              
              <button_1.Button variant="outline" size="sm" className="w-full justify-start" onClick={() => addNode('action')} data-testid="button-add-action">
                <i className="fas fa-cog text-accent mr-2"></i>
                Action
              </button_1.Button>
            </div>

            {selectedNode && (<div className="mt-6 p-3 bg-muted/50 rounded-lg">
          ...
          <div className="flex-1 relative">
            <div className="w-full h-full bg-muted/20 rounded-lg relative overflow-hidden cursor-crosshair border-2 border-dashed border-muted-foreground/20" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} data-testid="workflow-canvas">
              {/* Connection Lines */}
            ...
            if (!nextNode)
                return null;
            return (<line key={`line-${node.id}-${nextNode.id}`} x1={node.position.x + 80} y1={node.position.y + 25} x2={nextNode.position.x} y2={nextNode.position.y + 25} stroke="hsl(var(--muted-foreground))" strokeWidth="2" strokeDasharray="5,5" opacity="0.5"/>);
        })}
              </svg>

              {/* Workflow Nodes */}
              {nodes.map((node) => (<div key={node.id} className={`absolute cursor-move select-none transition-all duration-200 hover:scale-105 ${selectedNode === node.id ? 'ring-2 ring-primary' : ''}`} style={{
                left: node.position.x,
                top: node.position.y,
                transform: draggedNode === node.id ? 'scale(1.05)' : 'scale(1)'
            }} onMouseDown={(e) => handleMouseDown(e, node.id)} data-testid={`workflow-node-${node.id}`}>
