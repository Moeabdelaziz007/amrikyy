...
function KeyboardNavigation({ children, className, onNavigate, enabled = true }) {
    ...
    return (<div ref={containerRef} className={(0, utils_2.cn)(className)}>
      {children}
    </div>);
}
...
function SkipLink({ href, children, className }) {
    return (<a href={href} className={(0, utils_2.cn)("sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4", "z-50 px-4 py-2 bg-primary text-primary-foreground rounded-md", "neon-glow-sm transition-all duration-200", className)}>
      {children}
    </a>);
}
function KeyboardHint({ hint, className }) {
    return (<div className={(0, utils_2.cn)("text-xs text-muted-foreground bg-muted/50 px-2 py-1 rounded", "border border-border/50", className)}>
      {hint}
    </div>);
}
