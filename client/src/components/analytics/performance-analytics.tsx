...
import AdvancedAnalyticsService, { PerformanceMetric } from '../../lib/advanced-analytics-service';

interface PerformanceAnalyticsProps {
  userId: string;
}

export const PerformanceAnalytics: React.FC<PerformanceAnalyticsProps> = ({ userId }) => {
  ...
  const loadPerformanceMetrics = async () => {
    try {
      ...
      const data = await AdvancedAnalyticsService.monitorPerformanceMetrics(userId);
      setMetrics(data);
    } catch (err) {
  ...
  const getMetricIcon = (metric: string) => {
    switch (metric) {
      case 'session_duration': return <Clock className="h-5 w-5" />;
      case 'error_rate': return <AlertTriangle className="h-5 w-5" />;
      case 'engagement_score': return <Target className="h-5 w-5" />;
      case 'feature_adoption': return <Zap className="h-5 w-5" />;
      case 'response_time': return <Activity className="h-5 w-5" />;
      case 'memory_usage': return <HardDrive className="h-5 w-5" />;
      case 'cpu_usage': return <Cpu className="h-5 w-5" />;
      case 'network_latency': return <Wifi className="h-5 w-5" />;
  ...
  const getMetricStatus = (metric: PerformanceMetric) => {
    if (metric.percentile > 80) return { status: 'excellent', color: 'text-green-600', icon: CheckCircle };
    if (metric.percentile > 60) return { status: 'good', color: 'text-blue-600', icon: CheckCircle };
    if (metric.percentile > 40) return { status: 'fair', color: 'text-yellow-600', icon: Activity };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {metrics.slice(0, 4).map((metric) => {
          ...
          return (
            <Card key={metric.id} className="transition-all duration-200 hover:shadow-lg">
              ...
              <CardContent>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className={`text-sm font-medium ${getPercentileColor(metric.percentile)}`}>
      ...
      <Tabs defaultValue="overview" className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Visual representation of performance over time</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Metrics showing positive trends</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {metrics.filter(m => m.trend === 'improving').map((metric) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {metrics.filter(m => m.trend === 'declining').map((metric) => (
                    <div key={metric.id} className="flex items-center justify-between">
                      <div className="flex items-center space-x-2">
                        {getMetricIcon(metric.metric)}
                        <span className="text-sm font-medium capitalize">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-xs text-muted-foreground">
                            vs {formatMetricValue(metric.benchmark, metric.unit)}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center space-x-2">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-center space-y-4">
                  ...
                  <Progress 
                    value={metrics.reduce((sum, m) => sum + m.percentile, 0) / metrics.length} 
                    className="h-3" 
                  />
                  <div className="text-xs text-muted-foreground">
        ...
        <TabsContent value="alerts" className="space-y-6">
          <div className="space-y-4">
            {/* Performance Alerts */}
            {metrics.filter(m => m.percentile < 40 || m.trend === 'declining').map((metric) => (
              <Alert key={metric.id} variant={metric.percentile < 20 ? 'destructive' : 'default'}>
                <AlertTriangle className="h-4 w-4" />
                <AlertDescription>
                  <div className="flex items-center justify-between">
                    <div>
                      <strong>{metric.metric.replace('_', ' ')}</strong> requires attention
                      <div className="text-sm text-muted-foreground mt-1">
                        Current: {formatMetricValue(metric.value, metric.unit)} | 
                        Benchmark: {formatMetricValue(metric.benchmark, metric.unit)} | 
            ...
            {metrics.filter(m => m.percentile < 40 || m.trend === 'declining').length === 0 && (
              <Card>
                <CardContent className="flex flex-col items-center justify-center py-8">
                  <CheckCircle className="h-12 w-12 text-green-500 mb-4" />
                  <h3 className="text-lg font-medium mb-2">All Metrics Performing Well</h3>
                  <p className="text-muted-foreground text-center">
                    No performance alerts at this time. All metrics are within acceptable ranges.
                  </p>
                </CardContent>
              </Card>
            )}
