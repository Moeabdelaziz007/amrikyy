...
function UserHistoryAnalytics({ userId }) {
    ...
    const { analytics, loading: analyticsLoading, refresh: refreshAnalytics } = (0, use_user_history_1.useUserAnalytics)(selectedPeriod);
    const { sessions, loading: sessionsLoading } = (0, use_user_history_1.useUserSessions)();
    const { history, loading: historyLoading } = (0, use_user_history_1.useUserHistoryData)({ limit: 100 });
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <select value={selectedPeriod} onChange={(e) => setSelectedPeriod(e.target.value)} className="px-3 py-2 border rounded-md" aria-label="Select analytics period">
            <option value="daily">Daily</option>
            <option value="weekly">Weekly</option>
            <option value="monthly">Monthly</option>
            <option value="yearly">Yearly</option>
          </select>
          <button_1.Button onClick={refreshAnalytics} variant="outline" size="sm">
            <lucide_react_1.RefreshCw className="h-4 w-4 mr-2"/>
            Refresh
          </button_1.Button>
          <button_1.Button onClick={handleExportData} variant="outline" size="sm">
    ...
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <card_1.Card>
        <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <card_1.CardTitle className="text-sm font-medium">Total Actions</card_1.CardTitle>
      ...
      <card_1.Card>
        <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <card_1.CardTitle className="text-sm font-medium">Sessions</card_1.CardTitle>
      ...
      <card_1.Card>
        <card_1.CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <card_1.CardTitle className="text-sm font-medium">Retention</card_1.CardTitle>
      ...
      <card_1.Card className="col-span-1 md:col-span-2">
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>Top features by usage count</card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="space-y-4">
            {analytics?.stats?.mostUsedFeatures?.slice(0, 5).map((feature, index) => (<div key={feature.feature} className="space-y-2">
                <div className="flex items-center justify-between">
                ...
                <progress_1.Progress value={feature.percentage} className="h-2"/>
              </div>))}
          </div>
        </card_1.CardContent>
      </card_1.Card>
      ...
      <card_1.Card className="col-span-1 md:col-span-2">
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            {analytics?.stats?.topPages?.slice(0, 5).map((page, index) => (<div key={page.page} className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>Distribution of different action types</card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="space-y-3">
            ...
            .sort(([, a], [, b]) => b - a)
            .slice(0, 10)
            .map(([type, count]) => (<div key={type} className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
      ...
      <card_1.Card>
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>Distribution by category</card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="space-y-3">
      ...
      <card_1.Card className="col-span-1 lg:col-span-2">
        <card_1.CardHeader>
          ...
          <card_1.CardDescription>User activity throughout the day</card_1.CardDescription>
        </card_1.CardHeader>
        <card_1.CardContent>
          <div className="grid grid-cols-12 gap-2">
            ...
            return (<div key={hour} className="flex flex-col items-center space-y-1">
                  <div className="w-full bg-primary rounded-sm transition-all duration-300" style={{ height: `${Math.max(height, 5)}px` }}/>
...
function SessionsTab({ sessions }) {
    ...
    return (<div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <card_1.Card className="lg:col-span-2">
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            {sessions.slice(0, 10).map((session) => (<div key={session.id} className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedSession?.id === session.id ? 'bg-accent' : 'hover:bg-muted'}`} onClick={() => setSelectedSession(session)}>
                <div className="flex items-center justify-between">
                  <div>
                  ...
                  <div className="text-right">
                    <badge_1.Badge variant={session.isActive ? "default" : "secondary"}>
                      {session.isActive ? "Active" : "Ended"}
                    </badge_1.Badge>
                    {session.duration && (<div className="text-sm text-muted-foreground mt-1">
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-2">
          ...
          <select value={filter} onChange={(e) => setFilter(e.target.value)} className="px-3 py-1 border rounded-md text-sm" aria-label="Filter action categories">
        ...
        <div className="flex items-center space-x-2">
          ...
          <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search actions..." className="px-3 py-1 border rounded-md text-sm"/>
        </div>
      </div>
      ...
      <card_1.Card>
        ...
        <card_1.CardContent>
          <div className="space-y-4">
            {filteredHistory.slice(0, 50).map((action) => (<div key={action.id} className="flex items-center justify-between p-4 border rounded-lg">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    {action.errorMessage && (<div className="text-sm text-red-600 mt-1">
                ...
                <div className="text-right">
                  ...
                  {action.duration && (<div className="text-xs text-muted-foreground">
                      {action.duration}ms
                    </div>)}
                </div>
              </div>))}
