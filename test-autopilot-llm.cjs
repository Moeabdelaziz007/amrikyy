...
async function testBasicFunctionality() {
    ...
    try {
        ...
        if (missingVars.length === 0) {
            ...
            console.log(`   AUTOPILOT_LLM_ENABLED: ${process.env.AUTOPILOT_LLM_ENABLED}`);
            console.log(`   AUTOPILOT_LLM_PROVIDER: ${process.env.AUTOPILOT_LLM_PROVIDER}`);
            console.log(`   LLM_PROVIDER: ${process.env.LLM_PROVIDER}`);
        } else {
            console.log(chalk.red(`❌ Missing environment variables: ${missingVars.join(', ')}`));
            console.log(chalk.yellow('💡 Please configure these variables in your .env file'));
        }
        ...
        const llmScripts = Object.keys(packageJson.scripts).filter(script => 
            script.includes('autopilot') && script.includes('llm')
        );
...
async function testAutopilotLLMIntegration() {
    ...
    try {
        ...
        const testTask = {
            id: 'TEST_TASK_001',
            type: 'data_analysis',
            content: 'Analyze the sales data for the last quarter and provide insights',
            priority: 'high',
            status: 'pending',
        ...
        const response = await autopilotLLM.generateIntelligentResponse(query, context);
        ...
        console.log(`   Performance Improvements: ${optimization.performance_improvements.length} suggestions`);
        console.log(`   Efficiency Gains: ${optimization.efficiency_gains.length} suggestions`);

        // Test autopilot chat
        console.log(chalk.yellow('\n6. Testing autopilot chat...'));
        const chatResponse = await autopilotLLM.autopilotChat('What is the current status of my autopilot system?');
        console.log(chalk.green('✅ Autopilot chat completed:'));
        console.log(chalk.blue('Response:'), chatResponse.substring(0, 200) + '...');
        ...
        console.log(`   Performance Score: ${performanceAnalysis.performance_score}/100`);
        console.log(`   Recommendations: ${performanceAnalysis.recommendations.length} found`);
