...
class TaskDispatcher {
    ...
    initializeAgents() {
        ...
        this.agents.set('gemini_ai', {
            ...
            type: 'ai_analysis',
            capabilities: ['text_analysis', 'data_processing', 'content_generation', 'translation'],
            status: 'online',
            load: 0,
        ...
        this.agents.set('httpie_agent', {
            name: 'HTTPie Agent',
            type: 'web_automation',
            capabilities: ['web_scraping', 'api_calls', 'http_requests', 'data_extraction'],
            status: 'online',
            load: 0,
        ...
        this.agents.set('jq_agent', {
            ...
            capabilities: ['json_processing', 'data_filtering', 'data_transformation'],
            status: 'online',
            load: 0,
        ...
        this.agents.set('automation_agent', {
            ...
            type: 'automation',
            capabilities: ['task_automation', 'workflow_execution', 'system_commands'],
            status: 'online',
            load: 0,
        ...
        this.agents.set('file_agent', {
            ...
            capabilities: ['file_operations', 'directory_management', 'file_processing'],
            status: 'online',
            load: 0,
            maxConcurrentTasks: 5
        });
    ...
    async processTaskQueue() {
        ...
        for (let i = this.taskQueue.length - 1; i >= 0; i--) {
            ...
            if (agent && this.agents.get(agent).load < this.agents.get(agent).maxConcurrentTasks) {
    ...
    calculateAgentScore(agent, task) {
        ...
        if (categoryMatch[category] && categoryMatch[category].includes(agent.name.toLowerCase().replace(' ', '_'))) {
            score += 30;
        }
    ...
    addTask(task) {
        ...
        console.log(`📥 Task ${task.id} added to queue (${this.taskQueue.length} in queue)`);
    }
