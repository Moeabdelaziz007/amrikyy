...
class MessageBroker {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await redisClient.xGroupCreate(this.streamName, 'autopilot-group', '$', {
        MKSTREAM: true
      });
    } catch (error: any) {
      if (!error.message.includes('BUSYGROUP')) {
        logger.error('Autopilot Service: Failed to create consumer group:', error);
        throw error;
      }
    }
  }
  ...
  async subscribeToEvents(consumerName: string, callback: (event: EventSchema) => Promise<void>): Promise<void> {
    try {
          ...
          for (const stream of messages) {
            for (const message of stream.messages) {
              ...
              try {
                ...
                await redisClient.xAck(this.streamName, 'autopilot-group', message.id);
                logger.info(`Autopilot Service: Event processed: ${event.event_id}`);
              } catch (error) {
                logger.error(`Autopilot Service: Failed to process event ${event.event_id}:`, error);
              }
            }
          }
        }
      ...
      logger.error('Autopilot Service: Error in event subscription:', error);
      throw error;
    }
  }
  ...
  async publishEvent(event: Omit<EventSchema, 'event_id' | 'timestamp'>): Promise<string> {
    ...
    try {
      ...
      logger.info(`Autopilot Service: Event published: ${eventId}`, { event: fullEvent });
      return result;
    } catch (error) {
      logger.error('Autopilot Service: Failed to publish event:', error);
      throw error;
    }
  }
}
