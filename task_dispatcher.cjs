...
class TaskDispatcher {
    ...
    initializeAgents() {
        ...
        this.agents.set('gemini_ai', {
            ...
            type: 'ai_analysis',
            capabilities: [
                'text_analysis', 'data_processing', 'content_generation', 
        ...
        this.agents.set('httpie_agent', {
            ...
            type: 'web_automation',
            capabilities: [
                'web_scraping', 'api_calls', 'http_requests', 
                'data_extraction', 'website_monitoring'
            ],
            status: 'online',
            load: 0,
        ...
        this.agents.set('jq_agent', {
            ...
            capabilities: [
                'json_processing', 'data_filtering', 
        ...
        this.agents.set('automation_agent', {
            ...
            type: 'automation',
            capabilities: [
                'task_automation', 'workflow_execution', 
                'system_commands', 'process_automation'
            ],
            status: 'online',
            load: 0,
        ...
        this.agents.set('file_agent', {
            ...
            capabilities: [
                ...
                'file_processing', 'backup_operations'
            ],
            status: 'online',
            load: 0,
            maxConcurrentTasks: 5,
        ...
        this.agents.set('voice_processing_agent', {
            ...
            capabilities: [
                ...
                'voice_analysis', 'audio_processing'
            ],
            status: 'online',
            load: 0,
            maxConcurrentTasks: 3,
        ...
        this.agents.set('image_processing_agent', {
            ...
            capabilities: [
                'image_analysis', 'ocr', 'image_generation', 
                'image_enhancement', 'object_detection'
            ],
            status: 'online',
            load: 0,
    ...
    calculateAgentScore(agent, task) {
        ...
        if (categoryMatch[category] && categoryMatch[category].includes(agent.name.toLowerCase().replace(/\s+/g, '_'))) {
            score += 30;
        }
    ...
    updateAgentStats() {
        ...
        this.agentStats.forEach((stats, agentName) => {
            ...
            console.log(`  ${agentName}: ${stats.completedTasks}/${stats.totalTasks} (${successRate}%)`);
        });
    }
    ...
    checkAgentHealth() {
        this.agents.forEach((agent, name) => {
            ...
            if (agent.load > agent.maxConcurrentTasks * 0.9) {
                console.warn(`⚠️ Agent ${name} is overloaded: ${agent.load}/${agent.maxConcurrentTasks}`);
            }

            // فحص الكفاءة المنخفضة
            if (agent.efficiency < 0.7) {
                console.warn(`⚠️ Agent ${name} has low efficiency: ${agent.efficiency}`);
            }
        });
    }
    ...
    addTask(task) {
        ...
        console.log(`📥 Task ${task.id} added to dispatcher queue (${this.taskQueue.length} in queue)`);
    }
