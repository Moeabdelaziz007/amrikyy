...
class AutopilotTester {
  ...
  async testBasicConnectivity() {
    ...
    try {
      const response = await axios.get(
        `${BASE_URL}/api/automation/engine/stats`
      );
  ...
  async testAutopilotControlAPIs() {
    ...
    try {
      const stopResponse = await axios.post(
        `${BASE_URL}/api/autopilot/emergency-stop`,
        {
          stop: true,
        }
      );
  ...
  async testWorkflowControlAPIs() {
    ...
    try {
      ...
      const statsResponse = await axios.get(
        `${BASE_URL}/api/workflows/intelligent/stats`
      );
      ...
      if (statsResponse.status === 200 && statsResponse.data.workflows) {
        ...
        if (workflows.length > 0) {
          ...
          const pauseResponse = await axios.post(
            `${BASE_URL}/api/workflows/${firstWorkflow.id}/pause`
          );
          ...
          if (pauseResponse.status === 200 && pauseResponse.data.success) {
            ...
            const resumeResponse = await axios.post(
              `${BASE_URL}/api/workflows/${firstWorkflow.id}/resume`
            );
        ...
        this.wsConnection.on('message', data => {
          ...
          if (message.type === 'pong') {
            ...
            this.wsConnection.send(
              JSON.stringify({ type: 'get_autopilot_status' })
            );
  ...
  async testAIIntegrations() {
    ...
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/system/intelligence/overview`
      );
  ...
  async testEmergencyStop() {
    ...
    try {
      // Stop autopilot
      await axios.post(`${BASE_URL}/api/autopilot/emergency-stop`, {
        stop: true,
      });

      // Verify it's stopped
      const statusResponse = await axios.get(
        `${BASE_URL}/api/autopilot/live/status`
      );
      ...
      if (statusResponse.data.automation.emergencyStop === true) {
        ...
        // Verify it's resumed
        const resumeStatusResponse = await axios.get(
          `${BASE_URL}/api/autopilot/live/status`
        );
        ...
        if (resumeStatusResponse.data.automation.emergencyStop === false) {
          this.testResults.push({
            ...
            status: 'PASS',
            details:
              'Emergency stop and resume functionality working correctly',
          });
    ...
    try {
      ...
      if (rulesResponse.status === 200 && rulesResponse.data.totalRules > 0) {
        ...
        const overrideResponse = await axios.post(
          `${BASE_URL}/api/autopilot/rule/${mockRuleId}/override`,
          {
            override: {
              action: 'skip',
  ...
  generateTestReport() {
    ...
    if (successRate >= 90) {
      ...
      console.log(
        '   👍 GOOD - Autopilot system is mostly functional with minor issues'
      );
    } else if (successRate >= 50) {
      console.log(
        '   ⚠️ FAIR - Autopilot system has significant issues that need attention'
      );
    } else {
