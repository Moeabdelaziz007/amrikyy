...
class MessageBroker {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      await redisClient.xGroupCreate(this.streamName, 'gateway-group', '$', {
        MKSTREAM: true
      });
    } catch (error: any) {
  ...
  async publishEvent(event: Omit<EventSchema, 'event_id' | 'timestamp'>): Promise<string> {
    ...
    try {
      ...
      logger.info(`Event published: ${eventId}`, { event: fullEvent });
      return result;
    } catch (error) {
          ...
          for (const stream of messages) {
            for (const message of stream.messages) {
              ...
              try {
                ...
                await redisClient.xAck(this.streamName, 'gateway-group', message.id);
                logger.info(`Event processed: ${event.event_id}`);
              } catch (error) {
                logger.error(`Failed to process event ${event.event_id}:`, error);
              }
            }
          }
        }
  ...
  async getEventHistory(limit: number = 100): Promise<EventSchema[]> {
    try {
      const messages = await redisClient.xRevRange(this.streamName, '+', '-', {
        COUNT: limit
      });
