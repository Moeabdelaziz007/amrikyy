...
const DEFAULT_INTERVAL_MS = parseInt(
  process.env.AAPL_POLL_INTERVAL_MS || '60000'
);
...
export async function fetchQuote(symbol: string): Promise<StockQuote> {
  ...
  const change =
    ...
    result.postMarketChange ??
    result.preMarketChange ??
    0;
  const changePercent =
    result.regularMarketChangePercent ??
    result.postMarketChangePercent ??
    result.preMarketChangePercent ??
    0;
  return {
...
async function sendTelegram(message: string): Promise<void> {
  ...
  if (
    ...
    console.log(
      chalk.yellow('⚠️ Telegram not configured. Skipping notification.')
    );
    return;
  }
