  ...
  function analyzeLargeFiles(dirPath, basePath = '') {
    ...
    files.forEach(file => {
      ...
      if (stat.isFile() && stat.size > 100 * 1024) {
        ...
        largeFiles.push(
          ...analyzeLargeFiles(filePath, path.join(basePath, file))
        );
      }
    });
...
function testViteOptimizations() {
  ...
  optimizations.forEach(opt => {
    ...
    console.log(
      `${exists ? '✅' : '❌'} ${opt.name}: ${exists ? 'مفعل' : 'غير مفعل'}`
    );
  });
}
...
function evaluatePerformance() {
  ...
  // تقييم التحسينات
  const optimizationCount = Object.values(
    performanceResults.optimization
  ...
  if (totalOptimizations > 0) {
    ...
    console.log(
      `📈 تحسينات مفعلة: ${optimizationCount}/${totalOptimizations} (${(optimizationScore * 100).toFixed(1)}%)`
    );
  }
  ...
  if (
    performanceResults.bundleSize.totalSizeMB &&
    parseFloat(performanceResults.bundleSize.totalSizeMB) > 2
  ) {
