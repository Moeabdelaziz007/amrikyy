...
var CursorLLMIntegration = class {
  ...
  async sendMessage(message, context) {
    try {
      console.log(chalk.blue(`\u{1F916} Using ${this.config.provider} (${this.config.model})...`));
      switch (this.config.provider) {
        case "openai":
        case "groq":
        case "cursor":
...
program.name("cursor-llm").description("Cursor CLI with LLM Integration").version("2.0.0");
program.command("chat").description("Interactive chat with LLM").action(async () => {
  await llm.interactiveChat();
  process.exit(0);
});
program.command("analyze <file>").description("Analyze code file with LLM").action(async (file) => {
  await llm.analyzeCode(file);
  process.exit(0);
});
program.command("generate").description("Generate code with LLM").option("-l, --language <lang>", "Programming language", "typescript").option("-d, --description <desc>", "Code description").action(async (options) => {
...
program.command("ask <question>").description("Ask a quick question to LLM").action(async (question) => {
  try {
    console.log(chalk.blue("\u{1F914} Thinking..."));
...
var AutopilotLLMIntegration = class {
  ...
  async analyzeTask(task) {
    try {
      console.log(chalk2.blue(`\u{1F50D} Analyzing task ${task.id} with LLM...`));
      const prompt = `
      ...
      try {
        ...
        console.log(chalk2.green(`\u2705 Task ${task.id} analyzed successfully`));
        return analysis;
      } catch (parseError) {
        task.llmAnalysis = {
      ...
      console.error(chalk2.red(`\u274C Task analysis failed for ${task.id}:`), error);
      return {
        intent: "general task processing",
        category: "general",
        complexity: "medium",
...
User Context: ${userContext}`;
      ...
      console.error(chalk2.red("\u274C Intelligent response generation failed:"), error);
  ...
  async autopilotChat(query) {
    try {
      ...
      try {
        const response2 = await axios2.get(`${this.baseUrl}/api/autopilot/status`);
        autopilotContext = response2.data;
      } catch (error) {
        console.log(chalk2.yellow("\u26A0\uFE0F Could not fetch autopilot status for context"));
      }
      const contextPrompt = `
        User Query: ${query}
  ...
  async analyzeSystemPerformance() {
    try {
      console.log(chalk2.blue("\u{1F4CA} Analyzing system performance with LLM..."));
      ...
      console.error(chalk2.red("\u274C System performance analysis failed:"), error);
      return {
  ...
  async gatherSystemData() {
    ...
    try {
      const autopilotResponse = await axios2.get(`${this.baseUrl}/api/autopilot/status`);
      systemData.autopilot_status = autopilotResponse.data;
    } catch (error) {
      console.log(chalk2.yellow("\u26A0\uFE0F Could not fetch autopilot status"));
    }
    try {
      const systemResponse = await axios2.get(`${this.baseUrl}/api/system/status`);
  ...
  async interactiveChat() {
    ...
    console.log(chalk2.gray('Type "exit" to quit, "help" for commands, "analyze" for system analysis\n'));
    while (true) {
  ...
  async performSystemAnalysis() {
    ...
    try {
      ...
      console.log(`Performance Score: ${chalk2.blue(analysis.performance_score + "/100")}`);
      console.log(`Efficiency: ${chalk2.blue(analysis.key_metrics.efficiency)}`);
      console.log(`Reliability: ${chalk2.blue(analysis.key_metrics.reliability)}`);
      console.log(`Scalability: ${chalk2.blue(analysis.key_metrics.scalability)}`);
  ...
  async showAutopilotStatus() {
    ...
    try {
      ...
      console.log(`Status: ${status.active ? chalk2.green("\u2705 Active") : chalk2.red("\u274C Inactive")}`);
      console.log(`Active Rules: ${chalk2.blue(status.rules || 0)}`);
      console.log(`Active Workflows: ${chalk2.blue(status.workflows || 0)}`);
      console.log(`Last Execution: ${chalk2.yellow(status.lastExecution || "N/A")}`);
      console.log(`LLM Integration: ${this.config.enabled ? chalk2.green("\u2705 Enabled") : chalk2.red("\u274C Disabled")}`);
      console.log(`LLM Provider: ${chalk2.blue(this.config.defaultProvider)}`);
    } catch (error) {
      console.error(chalk2.red("\u274C Failed to get autopilot status:"), error.message);
    }
  }
  /**
   * Get configuration
...
program2.name("autopilot-llm").description("Autopilot LLM Integration System").version("1.0.0");
program2.command("chat").description("Interactive chat with autopilot LLM").action(async () => {
  await autopilotLLM.interactiveChat();
  process.exit(0);
});
program2.command("analyze").description("Analyze system performance with LLM").action(async () => {
  console.log(chalk2.blue.bold("\n\u{1F4CA} System Analysis\n"));
  try {
...
program2.command("ask <question>").description("Ask a question to the autopilot LLM").action(async (question) => {
  try {
...
program2.command("status").description("Show autopilot LLM status").action(async () => {
