...
export const Dashboard: React.FC = () => {
  ...
  const [logsClient, setLogsClient] = useState<{ id?: string; name?: string } | null>(null);
  const [darkMode, setDarkMode] = useState<boolean>(false);
  const [activeView, setActiveView] = useState<'dashboard' | 'workflow' | 'mcp' | 'activity' | 'settings'>('dashboard');
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (isMeta && e.shiftKey && key === 'l') {
        ...
        if (firstIdle) {
          setClients(prev => prev.map(c => c.id === firstIdle.id ? { ...c, status: 'running' as const } : c));
        }
        return;
      }
      ...
      if (isMeta && e.shiftKey && key === 's') {
        ...
        if (firstRunning) {
          setClients(prev => prev.map(c => c.id === firstRunning.id ? { ...c, status: 'idle' as const } : c));
        }
        return;
      }
    };
  ...
  return (
    <div className={`min-h-screen bg-bg-primary bg-cyber-grid bg-grid relative ${darkMode ? 'dark' : ''}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {clients.map((client) => (
                  <ClientCard
                    ...
                    progressPercent={client.status === 'running' ? Math.floor(Math.random() * 100) : undefined}
                    progressLabel={client.status === 'running' ? 'Processing...' : undefined}
