    ...
    default:
      return <span className={`icon-fallback ${className}`} style={{ fontSize: size }}>{fallback}</span>;
  }
};
...
interface IconProps {
  name?: string; // logical icon name, e.g. 'rocket', 'file-manager'
  fallback?: string; // emoji fallback or text
  size?: number; // px
  className?: string;
}
