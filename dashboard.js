...
class DashboardManager {
  ...
  setupSettingsListeners() {
    ...
    // Setting buttons
    document
      .getElementById('setup2FABtn')
      ?.addEventListener('click', () => this.setupTwoFactorAuth());
    document
      .getElementById('manageSessionsBtn')
      ?.addEventListener('click', () => this.manageSessions());
    document
      .getElementById('exportDataBtn')
      ?.addEventListener('click', () => this.exportUserData());
    document
  ...
  updateUserInterface() {
    ...
    if (profileAvatar) {
      profileAvatar.src =
        this.user.photoURL || 'https://via.placeholder.com/120';
    }
  ...
  updateStatsDisplay(stats) {
    ...
    if (sessionTime)
      sessionTime.textContent = this.formatTime(stats.sessionTime || 0);
  ...
  updateDailyCounter(cardId, success, failure) {
    ...
    if (successElement) {
      this.animateCounter(
        successElement,
        parseInt(successElement.textContent),
        success
      );
    }
    if (failureElement) {
      this.animateCounter(
        failureElement,
        parseInt(failureElement.textContent),
        failure
      );
    }
  }
    ...
    const existing = new Set(
      Array.from(
        document.querySelectorAll(
          'link[rel="prefetch"], link[rel="preload"], link[rel="preconnect"]'
        )
      ).map(l => l.href)
    );
    urls.forEach(u => {
...
if ('serviceWorker' in navigator) {
  window.addEventListener('load', () => {
    navigator.serviceWorker
      ...
      .then(registration => {
        console.log(
          'Service Worker registered for dashboard:',
          registration.scope
        );
      })
      .catch(error => {
