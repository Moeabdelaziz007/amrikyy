...
class AutopilotDashboard {
    ...
    setupEventListeners() {
        ...
        document.getElementById('cancelTask').addEventListener('click', () => this.closeModal('taskModal'));
        document.getElementById('cancelSettings').addEventListener('click', () => this.closeModal('settingsModal'));

        // Task creation
        document.getElementById('createTaskFab').addEventListener('click', () => this.openTaskModal());
        ...
        // Time range selector
        document.getElementById('timeRange').addEventListener('change', (e) => this.updateCharts(e.target.value));
    ...
    clearQueue() {
        if (confirm('هل أنت متأكد من مسح جميع المهام في قائمة الانتظار؟')) {
            document.getElementById('taskQueueList').innerHTML = '<div class="empty-state">قائمة الانتظار فارغة</div>';
    ...
    clearLogs() {
        if (confirm('هل أنت متأكد من مسح جميع السجلات؟')) {
            document.getElementById('systemLogs').innerHTML = '<div class="empty-state">لا توجد سجلات</div>';
    ...
    toggleLanguage() {
        ...
        this.showNotification(`تم تغيير اللغة إلى ${this.currentLanguage === 'ar' ? 'العربية' : 'English'}`, 'info');
    }
