...
(function () {
	...
	function ensureECharts() {
		...
		return loadScriptOnce('https://cdn.jsdelivr.net/npm/echarts@5/dist/echarts.min.js').then(() => window.echarts);
	}
	...
	function createLiveLineChart(el, options) {
		...
		function ensureChart() {
			return ensureECharts().then((echarts) => {
				...
				chart.setOption({
					...
					title: { text: state.opts.title || 'Live System Metrics', textStyle: { color: '#cbd5e1' } },
					tooltip: { trigger: 'axis' },
					legend: { textStyle: { color: '#94a3b8' } },
					xAxis: { type: 'category', boundaryGap: false, axisLabel: { color: '#94a3b8' } },
		...
		function pushPoint(name, value) {
			...
			if (state.data[name].length > (state.opts.maxPoints || DEFAULTS.maxPoints)) {
			...
			if (chart) {
				const series = (state.opts.series || []).map((s) => ({
					name: s.name,
					type: 'line',
					data: (state.data[s.name] || []).map(([x, y]) => ({ name: x, value: [x, y] })),
					symbol: s.symbol || 'circle',
					symbolSize: s.symbolSize || 6,
					smooth: true,
					showSymbol: false,
				...
				const x = (state.data[Object.keys(state.data)[0]] || []).map(([x]) => x);
	...
	function createAgentStatusWidget(el, options) {
		...
		statusEl.style.cssText = 'display:flex;align-items:center;gap:8px;color:#cbd5e1;';
		statusEl.innerHTML = `
		...
		async function tick() {
			try {
				...
				const res = await fetch(`${base}${DEFAULTS.restBase}/agents/${opts.agentId}/metrics`, { credentials: 'include' });
				if (!res.ok) throw new Error('metrics fetch failed');
				const { data } = await res.json();
				statusEl.querySelector('#label').textContent = `${data.name} (${data.status})`;
				statusEl.querySelector('#meta').textContent = `tasks=${data.performance.tasksCompleted} sr=${Math.round(data.performance.successRate)}%`;
				statusEl.querySelector('#dot').style.background = data.status === 'active' ? '#10b981' : data.status === 'learning' ? '#f59e0b' : data.status === 'error' ? '#ef4444' : '#64748b';
			} catch (e) {}
		}
		tick();
	...
	function createSystemLiveChart(el, wsClient) {
		...
		const unsub = wsClient.subscribe((msg) => {
			if (msg && msg.type === 'system_health' && msg.data && msg.data.components) {
		...
		return { dispose: () => { unsub(); chart.dispose(); } };
	}
