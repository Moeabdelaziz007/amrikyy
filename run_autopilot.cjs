...
class AutopilotBrain {
  ...
  async initializeLLM() {
    try {
      ...
      const { AutopilotLLMIntegration } = await import(
        './autopilot-llm-integration.js'
      );
...
class AuraOSAutopilot {
  ...
  connectComponents() {
    try {
      ...
      this.components.taskIntake.taskQueue =
        this.components.taskDispatcher.taskQueue;

      // ربط Task Dispatcher مع Task Executor
      this.components.taskDispatcher.taskExecutor =
        this.components.taskExecutor;

      // ربط Feedback Loop مع Task Dispatcher
      this.components.feedbackLoop.taskDispatcher =
        this.components.taskDispatcher;
  ...
  setupLoggingConnections() {
    ...
    this.components.taskIntake.logTask = (task, action) => {
      ...
      if (originalLogTask)
        originalLogTask.call(this.components.taskIntake, task, action);
    };

    // ربط تسجيل أداء الوكلاء
    const originalLogAgentPerformance =
      this.components.taskDispatcher.logAgentPerformance;
    this.components.taskDispatcher.logAgentPerformance = (
      agentName,
      performance
    ) => {
      this.components.logging.logAgentPerformance(agentName, performance);
      if (originalLogAgentPerformance)
        originalLogAgentPerformance.call(
          this.components.taskDispatcher,
          agentName,
          performance
        );
    ...
    const originalLogUserInteraction =
      this.components.taskIntake.logUserInteraction;
    this.components.taskIntake.logUserInteraction = (userId, interaction) => {
      ...
      if (originalLogUserInteraction)
        originalLogUserInteraction.call(
          this.components.taskIntake,
          userId,
          interaction
        );
  ...
  updateSystemStats() {
    if (this.startTime) {
      this.systemStats.uptime = Math.floor(
  ...
  checkSystemHealth() {
    ...
    if (unhealthyComponents.length > 0) {
      ...
      console.warn(
        `⚠️ Unhealthy components: ${unhealthyComponents.join(', ')}`
      );
  ...
  performSystemCleanup() {
    try {
      ...
      if (this.components.taskIntake?.taskHistory) {
        ...
        this.components.taskIntake.taskHistory =
          this.components.taskIntake.taskHistory.filter(task => {
      ...
      if (this.components.logging?.logBuffer) {
        this.components.logging.logBuffer =
          this.components.logging.logBuffer.slice(-1000);
      }
  ...
  getSystemStats() {
    return {
      ...
      components: {
        ...
        feedbackLoop:
          this.components.feedbackLoop?.getPerformanceMetrics() || {},
  ...
  async waitForBrainInitialization() {
    ...
    console.log(
      '⚠️ Brain initialization timeout - continuing without LLM integration'
    );
  }
  ...
  async performBrainAnalysis() {
    ...
    try {
      ...
      this.components.brain.updateSystemContext({
        currentLoad:
          this.systemStats.totalTasks > 10
            ? 80
            : this.systemStats.totalTasks * 8,
      ...
      const analysis =
        await this.components.brain.analyzeSystemPerformance(systemStats);
  ...
  async applyBrainRecommendations(recommendations) {
    for (const recommendation of recommendations.slice(0, 3)) {
      ...
      try {
        ...
        console.error(
          `❌ Failed to apply recommendation "${recommendation}":`,
          error
        );
      }
    }
