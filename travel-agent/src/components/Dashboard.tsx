...
const Dashboard: React.FC = () => {
  ...
  const quickStats = [
    { label: 'Active Bookings', value: '12', icon: Clock, color: 'text-blue-600' },
    { label: 'Saved Trips', value: '8', icon: Star, color: 'text-yellow-600' },
    { label: 'Travel Partners', value: '6', icon: Users, color: 'text-green-600' },
    { label: 'Countries Visited', value: '15', icon: Globe, color: 'text-purple-600' }
  ]

  const recentActivities = [
    { action: 'Booked flight to Paris', time: '2 hours ago', type: 'flight' },
    { action: 'Saved hotel in Tokyo', time: '1 day ago', type: 'hotel' },
    { action: 'AI suggested itinerary for Rome', time: '2 days ago', type: 'ai' },
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Integrated Travel Services</h2>
          ...
          {recentActivities.map((activity, index) => (
            <div key={index} className="flex items-center space-x-4 p-3 rounded-lg hover:bg-gray-50 transition-colors">
              <div className={`p-2 rounded-lg ${
                activity.type === 'flight' ? 'bg-blue-100' :
                activity.type === 'hotel' ? 'bg-green-100' :
                activity.type === 'ai' ? 'bg-purple-100' :
                'bg-orange-100'
              }`}>
                {activity.type === 'flight' && <Plane className="h-4 w-4 text-blue-600" />}
                {activity.type === 'hotel' && <Hotel className="h-4 w-4 text-green-600" />}
                {activity.type === 'ai' && <MessageCircle className="h-4 w-4 text-purple-600" />}
                {activity.type === 'car' && <Car className="h-4 w-4 text-orange-600" />}
              </div>
